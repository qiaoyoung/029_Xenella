...
@interface CropViewController : UIViewController<UITableViewDataSource,UITableViewDelegate,NIMLoginManagerDelegate,NIMConversationManagerDelegate>
/**
 *  会话列表tableView
 */
/**
...
@property (nonatomic,strong) NSMutableArray * managerBackground;

/**
 *  删除会话时是不是也同时删除服务器会话 (防止漫游)
 */
//: @property (nonatomic,assign) BOOL autoRemoveRemoteSession;
@property (nonatomic,assign) BOOL distant;
...
- (void)path:(NIMRecentSession *)recent
             //: atIndexPath:(NSIndexPath *)indexPath;
             mostDepth:(NSIndexPath *)indexPath;
//: @end
@end
