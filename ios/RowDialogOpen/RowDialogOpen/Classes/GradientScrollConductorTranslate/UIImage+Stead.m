...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(ends:));
...
+ (UIImage *)ends:(NSString *)name {
    ...
    UIImage *image = [[MinimumRecord extendBarrier] ends:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self ends:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
