...
@interface CropFifteenView : UIView

//: + (instancetype)shareMenuView;
+ (instancetype)shape;// 单例模式
...
- (void)infoPortrait:(NSArray *)array
                 ...
                 ratioRect:(CGRect)cursorStartRect name:(CGRect)rect startFound:(void(^)(AdminPaper *item))block
                     //: praiseBlock:(void(^)(NSInteger tag))praiseblock;
                     indicator:(void(^)(NSInteger tag))praiseblock;
