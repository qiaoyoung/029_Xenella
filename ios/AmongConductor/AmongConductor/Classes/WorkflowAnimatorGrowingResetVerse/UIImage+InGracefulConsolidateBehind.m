...
// __CLOSE_PRINT__
//
//  UIImage+InGracefulConsolidateBehind
//  NIM
//
...
//: @implementation UIImage (InGracefulConsolidateBehind)
@implementation UIImage (InGracefulConsolidateBehind)
//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)todayScene:(CGSize)newSize
{
...
- (UIImage *)infoDocument: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self tipPixels:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self tipPixels:suggestPixels];
    }
}


//: - (UIImage *)imageForAvatarUpload
- (UIImage *)isIndexUpload
{
    //: CGFloat pixels = [[HavenOpenLattice currentDevice] suggestImagePixels];
    CGFloat pixels = [[HavenOpenLattice currentDeviceDrop] eventEqual];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self infoDocument:pixels];
    //: return [image fixOrientation];
    return [image absoluteTo];
}

//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)tipPixels: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self todayScene:CGSizeMake(newSizeWidth, newSizeHeight)];
}
...
+ (UIImage *)monitorBasic:(NSString *)imageNameOrPath{
