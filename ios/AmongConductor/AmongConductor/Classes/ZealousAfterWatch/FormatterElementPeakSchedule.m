...
// __CLOSE_PRINT__
//
//  FormatterElementPeakSchedule.m
...
static NSString* const commonLightEvent (NSString *value) {
    if (value) {
        return [value.uppercaseString stringByAppendingString:@"end"];
    }
    return  @"\u2026";
};

//: @interface FormatterElementPeakSchedule ()
@interface FormatterElementPeakSchedule ()
//: @property (nonatomic,strong) FormatterElementPeakScheduleURL *touchedLink;
@property (nonatomic,strong) FormatterElementPeakScheduleURL *touchedLink;
...
//: @implementation FormatterElementPeakSchedule
@implementation FormatterElementPeakSchedule
...
- (NSAttributedString *)yearResume:(NSMutableAttributedString *)drawString
{
    //: if (drawString)
    if (drawString)
    {
...
- (NSMutableAttributedString*)safety:(NSMutableAttributedString *)wholeText {
...
- (void)setText:(NSString *)text
{
    ...
    NSAttributedString *attributedText = [self given:text];
...
- (NSAttributedString *)given:(NSString *)text
{
    //: if ([text length])
    if ([text length])
    {
...
+ (NSRegularExpression *)errorContainer {
...
- (NSMutableAttributedString *)lifeSized:(NSAttributedString *)attributedString {
    // 匹配 [表情]
    ...
    if (attrM.length > 0) {
        ...
        //: NSArray<NSTextCheckingResult *> *emoticonResults = [[FormatterElementPeakSchedule regexEmoticon] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
        NSArray<NSTextCheckingResult *> *emoticonResults = [[FormatterElementPeakSchedule errorContainer] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
    ...
    NSAttributedString *attString = [self yearResume:[[NSMutableAttributedString alloc] initWithAttributedString:attributedText]];
    //: _attributedString = [self needHightText:[self transformEmojiDescToEomjiImageWithAttributedString:attString]];
    _attributedString = [self safety:[self lifeSized:attString]];
