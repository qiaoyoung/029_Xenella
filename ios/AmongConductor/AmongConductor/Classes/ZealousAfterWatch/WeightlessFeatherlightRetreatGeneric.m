...
// __CLOSE_PRINT__
//
//  WeightlessFeatherlightRetreatGeneric.m
//  TianTianWang
//
...
//: @interface WeightlessFeatherlightRetreatGeneric ()
@interface WeightlessFeatherlightRetreatGeneric ()
...
//: @implementation WeightlessFeatherlightRetreatGeneric
@implementation WeightlessFeatherlightRetreatGeneric
...
- (void)streamFrame:(UIView *)dot below:(NSInteger)index
{
    // Dots are always centered within view
    //: CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self sizeForNumberOfPages:self.numberOfPages].width) / 2);
    CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index + ( (CGRectGetWidth(self.frame) - [self photo:self.numberOfPages].width) / 2);
...
- (void)playerBody:(BOOL)overrideExistingFrame
{
    //: CGPoint center = self.center;
    CGPoint center = self.center;
    //: CGSize requiredSize = [self sizeForNumberOfPages:self.numberOfPages];
    CGSize requiredSize = [self photo:self.numberOfPages];
...
- (void)conversation:(CGFloat)page
{
...
- (void)special
{
    ...
    for (NSInteger i = 0; i <= self.numberOfPages; i++) {
        ...
        //: } else {
            ...
            dot = [self belowCreate];
        }

        //: [self updateDotFrame:dot atIndex:i];
        [self streamFrame:dot below:i];
    }

    //: [self changeIndicatorViewLayout:self.currentPage];
    [self conversation:self.currentPage];
