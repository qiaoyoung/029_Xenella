...
// __CLOSE_PRINT__
//
//  PresenterDatasetterFormatterUpgradeUltimate.m
//  NIM
//
...
//: @implementation PresenterDatasetterFormatterUpgradeUltimate
@implementation PresenterDatasetterFormatterUpgradeUltimate

//: + (void)fileMD5:(NSString *)filepath completion:(void(^)(NSString *MD5))completion;
+ (void)being:(NSString *)filepath mediaDown:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self model:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)model:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self cart:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self standard:filepath];
        //: }else {
        }else {
...
+ (unsigned long long)cart:(NSString *)filepath
{
...
+ (NSString *)standard:(NSString *)filePath
{
