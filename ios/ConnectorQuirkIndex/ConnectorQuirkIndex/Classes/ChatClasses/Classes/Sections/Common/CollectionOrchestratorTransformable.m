...
// __CLOSE_PRINT__
//
//  CollectionOrchestratorTransformable.m
// ParseByBreakPerform
//
...
//: @interface CollectionOrchestratorTransformable ()
@interface CollectionOrchestratorTransformable ()
...
@property (nonatomic) CGFloat behind;
...
//: @implementation CollectionOrchestratorTransformable
@implementation CollectionOrchestratorTransformable

//: #pragma mark - Private
#pragma mark - Private
//: - (void)drawWithContent:(CGRect)rect context:(CGContextRef)context{
- (void)collection:(CGRect)rect reason:(CGContextRef)context{
    ...
    [[self badgeValue] drawInRect:CGRectMake(self.whiteCircleWidth + self.badgeLeftPadding,
                                             ...
                                             self.whiteCircleWidth + [self adjustment:self.badgeTopPadding],
...
- (void)guidance:(CGRect)rect figure:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    _badgeValue = badgeValue;
	[self setBehind:_badgeTopPadding];
    ...
    self.frame = [self yieldStr:badgeValue];
	[self setBehind:_badgeTopPadding];
...
+ (instancetype)earth:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    }
    //: CollectionOrchestratorTransformable *instance = [[CollectionOrchestratorTransformable alloc] init];
    CollectionOrchestratorTransformable *instance = [[CollectionOrchestratorTransformable alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance yieldStr:badgeValue];
...
- (CGSize)constantAcross:(NSString *)badgeValue{
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self collection:rect reason:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self guidance:rect figure:context];
    }
...
- (CGRect)yieldStr:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self constantAcross:badgeValue];
    ...
    CGRect badgeFrame = CGRectMake(self.frame.origin.x, self.frame.origin.y, badgeSize.width + self.badgeLeftPadding * 2 + self.whiteCircleWidth * 2, badgeSize.height + [self adjustment:self.badgeTopPadding] * 2 + self.whiteCircleWidth * 2);//8=2*2（红圈-文字）+2*2（白圈-红圈）
    //: return badgeFrame;
    return badgeFrame;
}

- (CGFloat)adjustment:(CGFloat)behind {
    //: OC_CUSTOM_PROPERTY_INJECT
    _behind = behind;
    return behind;
}
