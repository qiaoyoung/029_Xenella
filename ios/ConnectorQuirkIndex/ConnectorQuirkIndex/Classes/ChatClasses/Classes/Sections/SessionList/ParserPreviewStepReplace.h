...
//: @interface ParserPreviewStepReplace : UIViewController<UITableViewDataSource,UITableViewDelegate,NIMLoginManagerDelegate,NIMConversationManagerDelegate>
@interface ParserPreviewStepReplace : UIViewController<UITableViewDataSource,UITableViewDelegate,NIMLoginManagerDelegate,NIMConversationManagerDelegate>
/**
 *  会话列表tableView
 */
//: @property (nonatomic,strong) UITableView *tableView;
@property (nonatomic,strong) UITableView *imitator;

/**
 *  删除会话时是不是也同时删除服务器会话 (防止漫游)
 */
//: @property (nonatomic,assign) BOOL autoRemoveRemoteSession;
@property (nonatomic,assign) BOOL portion;
 ...
 *  @return 时间戳格式化后的字符串
 *
 *  @discussion 默认实现为：最后一条消息的时间戳，具体时间戳消息格式详见AmongVisualizeHill中， + (NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail 方法的实现。
 */
//: - (NSString *)timestampDescriptionForRecentSession:(NIMRecentSession *)recent;
- (NSString *)rafterSession:(NIMRecentSession *)recent;
...
- (void)lab:(NIMRecentSession *)recent
             //: atIndexPath:(NSIndexPath *)indexPath;
             hill:(NSIndexPath *)indexPath;
/**
