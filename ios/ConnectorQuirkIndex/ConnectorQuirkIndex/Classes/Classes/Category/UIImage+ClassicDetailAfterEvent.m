...
// __CLOSE_PRINT__
//
//  UIImage+ClassicDetailAfterEvent
//  NIM
//
...
//: @implementation UIImage (ClassicDetailAfterEvent)
@implementation UIImage (ClassicDetailAfterEvent)
//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)common: (CGFloat)maxPixels
{
    ...
    return [self selfLocking:CGSizeMake(newSizeWidth, newSizeHeight)];
}


//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)selfLocking:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self capabilityPerson:size];
}


//: - (UIImage *)imageForAvatarUpload
- (UIImage *)fadePersistUpload
{
    //: CGFloat pixels = [[ThreadedValidateSteamComposer currentDevice] suggestImagePixels];
    CGFloat pixels = [[ThreadedValidateSteamComposer insert] epicanthus];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self painter:pixels];
    //: return [image fixOrientation];
    return [image worldPowerSurf];
}

//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)capabilityPerson: (CGSize)size
{
...
+ (UIImage *)ledgeCreature:(NSString *)imageNameOrPath{
...
- (UIImage *)painter: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self common:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self common:suggestPixels];
    }
}
