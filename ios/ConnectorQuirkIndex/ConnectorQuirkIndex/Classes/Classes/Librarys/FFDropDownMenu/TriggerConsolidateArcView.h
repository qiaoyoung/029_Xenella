...
//: @class TriggerConsolidateArcView;
@class TriggerConsolidateArcView;
 ...
 *  菜单展现的动画类型
 */
//: typedef NS_ENUM(NSInteger, TriggerConsolidateArcViewAnimateType) {
typedef NS_ENUM(NSInteger, TriggerConsolidateArcViewAnimateType) {
    /**
     *  scale based the point of upper right corner.
     *  以右上角为基点进行伸缩
     */
    //: TriggerConsolidateArcViewAnimateType_ScaleBasedTopRight = 0, 
    TriggerConsolidateArcViewAnimateType_ScaleBasedTopRight = 0, //default
    /**
     *  scale based the point of upper left corner.
     *  以左上角为基点进行伸缩
     */
    //: TriggerConsolidateArcViewAnimateType_ScaleBasedTopLeft,
    TriggerConsolidateArcViewAnimateType_ScaleBasedTopLeft,
    /**
     *  scale based the middle point
     *  以中点为基点进行伸缩
     */
    //: TriggerConsolidateArcViewAnimateType_ScaleBasedMiddle,
    TriggerConsolidateArcViewAnimateType_ScaleBasedMiddle,
    /**
     *  fade in fade out
     *  淡入淡出效果
     */
    //: TriggerConsolidateArcViewAnimateType_FadeInFadeOut,
    TriggerConsolidateArcViewAnimateType_FadeInFadeOut,
    /**
     *  animate like RollerShutter
     *  卷帘效果
     */
    //: TriggerConsolidateArcViewAnimateType_RollerShutter,
    TriggerConsolidateArcViewAnimateType_RollerShutter,
    /**
     *  fall from top
     *  从上往下落下
     */
    //: TriggerConsolidateArcViewAnimateType_FallFromTop,
    TriggerConsolidateArcViewAnimateType_FallFromTop,
//: };
};


//: @protocol TriggerConsolidateArcViewDelegate <NSObject>
@protocol TriggerConsolidateArcViewDelegate <NSObject>
...
//: - (void)TriggerConsolidateArcView:(TriggerConsolidateArcView *)menuView WillAppearMenuCell:(TriggerConsolidateArcBasedCell *)menuCell index:(NSInteger)index;
- (void)portrait:(TriggerConsolidateArcView *)menuView galaxy:(TriggerConsolidateArcBasedCell *)menuCell menu:(NSInteger)index;

//: - (void)TriggerConsolidateArcViewWillAppear;
- (void)monthText;

//: - (void)TriggerConsolidateArcViewWDidAppear;
- (void)limitAcrossAppear;

//: - (void)TriggerConsolidateArcViewWillDisappear;
- (void)emptyPop;

//: - (void)TriggerConsolidateArcViewWDidDisappear;
- (void)expectedEdge;
 ...
 *  下拉菜单
 */
//: @interface TriggerConsolidateArcView : UIView
@interface TriggerConsolidateArcView : UIView
 ...
 *
 *
 *  @param menuModelsArray     下拉菜单模型数组,数组中存放着(TriggerConsolidateArcModel模型对象)
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage TriggerConsolidateArcModel instances
...
@property (nonatomic, assign) CGFloat word;
/** 18、delegate
 *
 */
//: @property (nonatomic, weak) id<TriggerConsolidateArcViewDelegate> delegate;
@property (nonatomic, weak) id<TriggerConsolidateArcViewDelegate> uponBehaviorEnrichAccelerates;
...
@property (nonatomic, assign) CGSize chromatic;
...
@property (nonatomic, assign) CGFloat root;
...
@property (nonatomic, assign) CGFloat givenExternalMargin;
...
@property (nonatomic, strong) UIColor *counternational;

@property (nonatomic, assign) CGFloat might;
...
///   >>步骤:
///   1、 [TriggerConsolidateArcView alloc] init]
///   2、 对需要赋值的属性进行赋值
...
@property (nonatomic, assign) UIColor *extendExist;
...
@property (nonatomic, assign) CGFloat refreshAsset;
...
@property (nonatomic, strong) UIColor *picture;

/** 15、菜单的弹出的动画效果类型(若不设置，默认为TriggerConsolidateArcViewAnimateType_ScaleBasedTopRight)
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     show drop-down menu animate type. (if not assign, default value is TriggerConsolidateArcViewAnimateType_ScaleBasedTopRight)
 */
//: @property (nonatomic, assign) TriggerConsolidateArcViewAnimateType menuAnimateType;
@property (nonatomic, assign) TriggerConsolidateArcViewAnimateType clear;
...
@property (nonatomic, assign) CGFloat rowFloat;

/** 2、cell的类名, 必须是TriggerConsolidateArcBasedCell的子类
 *               若用框架自带的cell,直接传@"TriggerConsolidateArcCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"TriggerConsolidateArcCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of TriggerConsolidateArcBasedCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"TriggerConsolidateArcCell"
...
@property (nonatomic, assign) CGFloat island;

@property (nonatomic, assign) UIColor *quantity;
...
/** 1、下拉菜单模型数组(数组里面存放 TriggerConsolidateArcBasedModel的子类的对象) 
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage TriggerConsolidateArcModel instances or the TriggerConsolidateArcModel's subclass instances )
 */
...
@property (nonatomic, assign) CGFloat conformToMargin;
...
@property (nonatomic, assign) CGFloat menu;
