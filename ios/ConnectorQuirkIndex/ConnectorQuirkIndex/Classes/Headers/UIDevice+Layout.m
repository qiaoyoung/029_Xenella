...
+ (CGFloat)noWorkerHeight {
    //: return [UIDevice vg_statusBarHeight] + [UIDevice vg_navigationBarHeight];
    return [UIDevice chemical] + [UIDevice definiteDrag];
}
...
+ (CGFloat)uniqueBy {
    ...
    return [UIDevice chemical] + [UIDevice utilizer];
}
...
+ (CGFloat)chemical {
    ...
    if (statusBarHeight <= 0) {
        ...
        CGFloat topPadding = [UIDevice span];
