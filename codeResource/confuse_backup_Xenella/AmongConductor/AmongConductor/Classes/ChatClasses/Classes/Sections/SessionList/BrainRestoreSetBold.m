...
@interface BrainRestoreSetBold ()

//@property (nonatomic,strong)  UIImageView *navBarHairlineImageView;

@end

@implementation BrainRestoreSetBold
...
- (void)tapGestureRecognizer:(id)sender {
    BelowAfterFramework *vc = [[BelowAfterFramework alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    WriteConfigClip *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    if (!cell) {
        cell = [[WriteConfigClip alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
...
- (void)onSelectedRecent:(NIMRecentSession *)recentSession atIndexPath:(NSIndexPath *)indexPath{
    VoyageMagicalWriteWreath *vc = [[VoyageMagicalWriteWreath alloc] initWithSession:recentSession.session];
...
- (NSString *)nameForRecentSession:(NIMRecentSession *)recent {
    if (recent.session.sessionType == NIMSessionTypeP2P) {
        return [HoldSheetCalculate showNick:recent.session.sessionId inSession:recent.session];
...
- (NSString *)timestampDescriptionForRecentSession:(NIMRecentSession *)recent{
    if (recent.lastMessage) {
        return [HoldSheetCalculate showTime:recent.lastMessage.timestamp showDetail:NO];
    }
    // 服务端时间戳以毫秒为单位,需要转化
    NSTimeInterval timeSecond = recent.updateTime / 1000.0;
    return [HoldSheetCalculate showTime:timeSecond showDetail:NO];
}
...
- (NSString *)messageContent:(NIMMessage*)lastMessage{
    NSString *text = [VirtuousRegisterRemove messageContent:lastMessage];
    ...
    else
    {
        ...
        text = [VirtuousRegisterRemove messageContent:msg];
        
        NSString *from = msg.from;
        NSString *nickName = [HoldSheetCalculate showNick:from inSession:msg.session];
