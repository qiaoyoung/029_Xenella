//
//  SignerPreviewCaptureTimeline.m
// TreatLayoutExotic
//
...
static NSString * const kNIMListCellReuseID = @"AboveGetAgentThorough";
static const CGFloat kNIMAdvancedBackgroundPadding = 5;

@interface SignerPreviewCaptureTimeline () <UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
...
@implementation SignerPreviewCaptureTimeline
...
- (void)refreshtranslationView:(DeriveFixAccess *)data
{
...
- (void)refreshEmoticonsView:(DeriveFixAccess *)data
{
    self.objects = nil;
    self.map = nil;
    ...
    self.map =  result;
    // 按最近评论优先排序
    self.objects = [AbstractFeatherlightScope sortedKeys:result];
...
- (void)refreshCollection:(DeriveFixAccess *)data
{
    if ([data needShowEmoticonsView])
    {
        ...
        if (!collectionView)
        {
            UICollectionViewFlowLayout *flowLayout = [[FilterPainterUnderModalLayout alloc] init];
            ...
            [collectionView registerClass:[AboveGetAgentThorough class] forCellWithReuseIdentifier:kNIMListCellReuseID];
...
- (void)refreshPinView:(DeriveFixAccess *)data
{
...
- (void)refreshReplyCountView:(DeriveFixAccess *)data
{
...
{
    AboveGetAgentThorough *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kNIMListCellReuseID forIndexPath:indexPath];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    NSNumber *number = [self.objects objectAtIndex:indexPath.item];
    NSArray *comments = [self.map objectForKey:number];
    CGSize size = [AbstractFeatherlightScope itemSizeWithComments:comments];
    return size;
}
...
{
    NIMQuickComment *comment = [AbstractFeatherlightScope myCommentFromComments:indexPath.item
                                                                 keys:self.objects
                                                             comments:self.map];
