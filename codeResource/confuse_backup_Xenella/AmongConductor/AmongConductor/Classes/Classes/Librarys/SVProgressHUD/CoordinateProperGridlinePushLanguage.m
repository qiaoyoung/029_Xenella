...
#error CoordinateProperGridlinePushLanguage is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
+ (CoordinateProperGridlinePushLanguage*)sharedView {
    ...
    static CoordinateProperGridlinePushLanguage *sharedView;
     ...
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[CoordinateProperGridlinePushLanguage mainWindow].bounds]; });
#else
...
- (void)updateHUDFrame {
    ...
    hudWidth = CoordinateProperGridlinePushLanguageHorizontalSpacing + MAX(labelWidth, contentWidth) + CoordinateProperGridlinePushLanguageHorizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(CoordinateProperGridlinePushLanguageVerticalSpacing, (self.minimumSize.height - contentHeight - CoordinateProperGridlinePushLanguageLabelSpacing - labelHeight) / 2.0f);
...
#else
            // If CoordinateProperGridlinePushLanguage is used inside an app extension add it to the given view
