...
- (void)remoteUpdateGender{
    ...
    [TupleApplyCompute getWithUrl:Server_user_editinfo params:dict isShow:YES success:^(id responseObject) {
...
- (ConsumeTimeMount *)changeNickNameView
{
    if(!_changeNickNameView){
        _changeNickNameView = [[ConsumeTimeMount alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_sexView){
        _sexView = [[DatasetterBalancerWillow alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
//        _sexView.hidden = YES;
    }
    return _sexView;
}
