...
//    dict[@"user_id"] = self.userId;
    [TupleApplyCompute getWithUrl:Server_user_recommend params:nil isShow:NO success:^(id responseObject) {
...
- (void)addFriend:(NSString *)userId
{
    ...
    [TupleApplyCompute getWithUrl:Server_user_search params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
...
-(void)checkIsAdmin:(NSString *)userId{
    ...
    [TupleApplyCompute getWithUrl:Server_user_checkAddFriendEx params:dict isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue == 0) {
            //跳转详情
...
{
    if(!_MyQRcodeView){
        _MyQRcodeView = [[FormatterThroughoutJubilant alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
