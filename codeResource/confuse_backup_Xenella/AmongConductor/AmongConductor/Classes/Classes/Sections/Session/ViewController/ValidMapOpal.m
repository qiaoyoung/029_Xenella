...
//    }
    ...
    [TupleApplyCompute getWithUrl:Server_team_getTeamSetting params:dict isShow:NO success:^(id responseObject) {
        @strongify(self)
...
//            }
    ...
    [TupleApplyCompute getWithUrl:Server_team_getStatusSendText params:dict isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
    ...
    self.header = [[TallRemoveTabviewTheory alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(CalibrateReturnDevice *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [CoordinateProperGridlinePushLanguage showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [CoordinateProperGridlinePushLanguage showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(CalibrateReturnDevice *)item
{
    ...
    [TupleApplyCompute getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[LoadLatticeHill alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)didTouchSubmitContentButton:(NSString *)reason
{
    ...
    if(isMyFriend){
        ...
        NSMutableDictionary *dict = @{}.mutableCopy;
        dict[@"contact"] = reason;
        [TupleApplyCompute postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[WorkActionSilverCohesiveNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[WorkActionSilverCohesiveBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (WorkActionSilverCohesiveDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[WorkActionSilverCohesiveDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (WorkActionSilverCohesiveView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[WorkActionSilverCohesiveView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
