...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [UpComposerLedgeMind passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [UpComposerLedgeMind setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
