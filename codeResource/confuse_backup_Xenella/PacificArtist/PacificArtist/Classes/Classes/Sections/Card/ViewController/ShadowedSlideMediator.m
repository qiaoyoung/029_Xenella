...
//    [self setUpNav];
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [ShapeSurfTerminalSystematic show];
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
...
-(void)addFriend{
    ...
    if (_teamSetingConfig != nil) {
        ...
        if (canAddFriend.integerValue <= 0) {
            [ShapeSurfTerminalSystematic showMessage:LangKey(@"please_contact_your_administrator")];
            return;
        }
    }
...
-(void)setingGroup{
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (BrokerSchedulerDecoder *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[BrokerSchedulerDecoder alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotView){
        _reprotView  = [[MapConsumeCollectorElevatedUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
