...
- (void)needsInitUI {
    ...
    if (userDefaults.language && userDefaults.language.length <= 0) {
        ...
        [FertileSuiteEnableCacheLine refreshGlobalConfig:^(NSDictionary * _Nonnull configDict) {//处理第一次启动语言问题
...
//    }
    ...
    @weakify(self);
    [FertileSuiteEnableCacheLine getWithUrl:Server_user_login params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_loadingView){
        _loadingView = [[TimelessStairBy alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
