...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [RevokeAtSaverSynchronizerTopmost passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [RevokeAtSaverSynchronizerTopmost setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
