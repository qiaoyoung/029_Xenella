...
@property (nonatomic, strong) SignatureCoolAmongProjectorView *dropdownMenu;/** 下拉菜单 */
...
-(void)checkCreateTeam{
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    //菜单模型1
    @weakify(self);
    SignatureCoolAmongProjectorModel *menuModel3 = [SignatureCoolAmongProjectorModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
{
    if(!_noticeView){
        _noticeView = [[HoldRestoreDomePremier alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
