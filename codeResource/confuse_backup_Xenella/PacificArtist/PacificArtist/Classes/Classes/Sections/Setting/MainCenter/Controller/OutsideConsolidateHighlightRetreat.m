...
- (void)remoteUpdateGender{
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_user_editinfo params:dict isShow:YES success:^(id responseObject) {
...
- (ManageSchedulerAnalyzeMinimalCoder *)changeNickNameView
{
    if(!_changeNickNameView){
        _changeNickNameView = [[ManageSchedulerAnalyzeMinimalCoder alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_sexView){
        _sexView = [[CoolForReturn alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
//        _sexView.hidden = YES;
    }
    return _sexView;
}
