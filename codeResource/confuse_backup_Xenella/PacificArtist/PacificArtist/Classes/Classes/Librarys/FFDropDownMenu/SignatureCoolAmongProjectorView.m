    ...
    if (_tableView == nil) {
        ...
        if ([self.cellClassName hasSuffix:@".xib"]) { //xib名称
            ...
            if (![NSClassFromString(className) isSubclassOfClass:[SignatureCoolAmongProjectorBasedCell class]]) {
                FFLog(@"%@这个类不是SignatureCoolAmongProjectorBasedCell的子类,请继承这个类",className);
                return _tableView;
            }
            ...
            if (![NSClassFromString(self.cellClassName) isSubclassOfClass:[SignatureCoolAmongProjectorBasedCell class]]) {
                FFLog(@"%@这个类不是SignatureCoolAmongProjectorBasedCell的子类,请继承这个类",self.cellClassName);
                return _tableView;
            }
...
- (void)setMenuAnimateType:(SignatureCoolAmongProjectorViewAnimateType)menuAnimateType { //15
