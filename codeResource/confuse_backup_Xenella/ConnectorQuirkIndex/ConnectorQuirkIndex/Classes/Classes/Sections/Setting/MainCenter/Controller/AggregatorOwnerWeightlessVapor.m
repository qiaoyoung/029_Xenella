...
- (void)remoteUpdateGender{
    ...
    [LocalizeHelixAccept getWithUrl:Server_user_editinfo params:dict isShow:YES success:^(id responseObject) {
...
- (TransitViewportParserParse *)changeNickNameView
{
    if(!_changeNickNameView){
        _changeNickNameView = [[TransitViewportParserParse alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_sexView){
        _sexView = [[AxisLocalizeContourPainterInlet alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
//        _sexView.hidden = YES;
    }
    return _sexView;
}
