...
@property (nonatomic, strong) TriggerConsolidateArcView *dropdownMenu;/** 下拉菜单 */
...
-(void)checkCreateTeam{
    ...
    [LocalizeHelixAccept getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    TriggerConsolidateArcModel *menuModel1 = [TriggerConsolidateArcModel ff_DropDownMenuModelWithMenuItemTitle:activity menuItemIconName:@"ic_create_chat" menuBlock:^{
        if (_creatTeam.integerValue != 0) {
            [ResizeNucleusDocumentRow showMessage:LangKey(@"please_contact_your_administrator")];
        }else{
    ...
    //菜单模型1
    @weakify(self);
    TriggerConsolidateArcModel *menuModel3 = [TriggerConsolidateArcModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
{
    if(!_noticeView){
        _noticeView = [[KindRegistryWinterMoor alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
