...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [VisitorSignerRoyalDelegate passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [VisitorSignerRoyalDelegate setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
