...
//    }
    ...
    self.header = [[KeyPleasantMainFont alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(ZoneToolbarCompress *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [ResizeNucleusDocumentRow showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [ResizeNucleusDocumentRow showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(ZoneToolbarCompress *)item
{
    ...
    [LocalizeHelixAccept getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[PromiseAnimateWindowContinue alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)didTouchSubmitContentButton:(NSString *)reason
{
    ...
    if(isMyFriend){
        ...
        NSMutableDictionary *dict = @{}.mutableCopy;
        dict[@"contact"] = reason;
        [LocalizeHelixAccept postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[SoundUpNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[SoundUpBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (SoundUpDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[SoundUpDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (SoundUpView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[SoundUpView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
