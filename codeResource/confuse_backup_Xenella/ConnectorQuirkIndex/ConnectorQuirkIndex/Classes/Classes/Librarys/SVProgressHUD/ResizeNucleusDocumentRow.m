...
#error ResizeNucleusDocumentRow is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
+ (ResizeNucleusDocumentRow*)sharedView {
    ...
    static ResizeNucleusDocumentRow *sharedView;
     ...
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[ResizeNucleusDocumentRow mainWindow].bounds]; });
#else
...
- (void)updateHUDFrame {
    ...
    hudWidth = ResizeNucleusDocumentRowHorizontalSpacing + MAX(labelWidth, contentWidth) + ResizeNucleusDocumentRowHorizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(ResizeNucleusDocumentRowVerticalSpacing, (self.minimumSize.height - contentHeight - ResizeNucleusDocumentRowLabelSpacing - labelHeight) / 2.0f);
...
#else
            // If ResizeNucleusDocumentRow is used inside an app extension add it to the given view
