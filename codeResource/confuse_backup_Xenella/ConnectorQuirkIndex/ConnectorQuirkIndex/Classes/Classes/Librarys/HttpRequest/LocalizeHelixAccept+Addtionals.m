...
+ (void)refreshRegistConfig:(GlobalConfigBlock)block{
    ...
    [LocalizeHelixAccept getWithUrl:Server_regist_config params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshGlobalConfig:(GlobalConfigBlock)block{
    ...
    [LocalizeHelixAccept getWithUrl:Server_regist_appSetting params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshGetBookStatus:(GlobalConfigBlock)block{
    [LocalizeHelixAccept getWithUrl:Server_user_getBookStatus params:nil isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        block(resultDict);
    } failed:^(id responseObject, NSError *error) {
        block(@{});
...
+ (void)refreshGenerateUser:(GlobalConfigBlock)block {
    [LocalizeHelixAccept getWithUrl:Server_regist_generateUser params:nil isShow:NO success:^(id responseObject) {
...
+(void)refreshCheckmoments:(GlobalConfigBlock)block{
    [LocalizeHelixAccept getWithUrl:Server_user_checkmoments params:nil isShow:NO success:^(id responseObject) {
        
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            [CurrentSearchComposer standardUserDefaults].friendsCircle = kConstant_1;
        }else{
            [CurrentSearchComposer standardUserDefaults].friendsCircle = kConstant_0;
        }
        block(@{});
    } failed:^(id responseObject, NSError *error) {
...
+(void)refreshSystemSetting:(GlobalConfigBlock)block{
    [LocalizeHelixAccept getWithUrl:Server_other_systemSetting params:nil isShow:NO success:^(id responseObject) {
...
+(void)deleteUser:(GlobalConfigBlock)block{
    ...
    [LocalizeHelixAccept getWithUrl:Server_deleteUser_new params:params isShow:YES success:^(id responseObject) {
