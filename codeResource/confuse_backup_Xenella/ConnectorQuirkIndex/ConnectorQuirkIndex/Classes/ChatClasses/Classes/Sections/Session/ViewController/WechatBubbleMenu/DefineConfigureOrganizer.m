...
typedef NS_OPTIONS(NSUInteger, BufferHarmlessAround) {
    BufferHarmlessAround_Up,
    BufferHarmlessAround_Down
};

@interface DefineConfigureOrganizer()
...
@property (nonatomic, assign)BufferHarmlessAround directionPriority;
...
@implementation DefineConfigureOrganizer

+ (instancetype)shareMenuView {
    static DefineConfigureOrganizer *menu = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (menu == nil) {
            menu = [[DefineConfigureOrganizer alloc] init];
        }
    });
  return menu;
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _directionPriority = BufferHarmlessAround_Up;
...
- (void)showViewWithButtonModels:(NSArray *)array
                 cursorStartRect:(CGRect)cursorStartRect selectionTextRectInWindow:(CGRect)rect selectBlock:(void(^)(ZoneToolbarCompress *item))block praiseBlock:(void(^)(NSInteger tag))praiseblock {
    ...
    //默认的方向是优先方向
    BufferHarmlessAround direction = _directionPriority;
//
...
//        //优先级Up的话，先判断在上面的情况。
        ...
        if (rect.origin.y - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            direction = BufferHarmlessAround_Up;
            ...
            direction = BufferHarmlessAround_Down;
...
//    }
    ...
    bgViewRect.origin.y = direction == BufferHarmlessAround_Down ? _arrowHeight : praiseheight;
...
//    }
    ...
    if (direction == BufferHarmlessAround_Up) {
...
- (UIView *)drawButtonsWithArray:(NSArray *)array {
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        ShellEnqueueUnifiedOpen *model = array[i];
...
- (void)onButtonTouched:(UIButton *)button {
    if (_myNewContentArray.count > button.tag - 100) {
        ShellEnqueueUnifiedOpen *model = (ShellEnqueueUnifiedOpen *)_myNewContentArray[button.tag - 100];
...
- (BOOL)ifButtonsInfoChanged:(NSArray *)newArray {
    ...
    for (int i = 0; i < newArray.count; i ++) {
        BOOL finded = false;
        NSInteger newId = ((ShellEnqueueUnifiedOpen *)_oldContentArray[i]).buttonId;
        for (int j = 0; j < _oldContentArray.count; j ++) {
            NSInteger oldId = ((ShellEnqueueUnifiedOpen *)_oldContentArray[j]).buttonId;
            if (newId == oldId) {
                finded = true;
                continue;
            }
