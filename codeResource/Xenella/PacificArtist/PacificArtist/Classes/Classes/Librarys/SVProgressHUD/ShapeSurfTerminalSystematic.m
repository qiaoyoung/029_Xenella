...
#error ShapeSurfTerminalSystematic is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
+ (ShapeSurfTerminalSystematic*)sharedView {
    ...
    static ShapeSurfTerminalSystematic *sharedView;
     ...
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[ShapeSurfTerminalSystematic mainWindow].bounds]; });
#else
...
- (void)updateHUDFrame {
    ...
    hudWidth = ShapeSurfTerminalSystematicHorizontalSpacing + MAX(labelWidth, contentWidth) + ShapeSurfTerminalSystematicHorizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(ShapeSurfTerminalSystematicVerticalSpacing, (self.minimumSize.height - contentHeight - ShapeSurfTerminalSystematicLabelSpacing - labelHeight) / 2.0f);
...
#else
            // If ShapeSurfTerminalSystematic is used inside an app extension add it to the given view
