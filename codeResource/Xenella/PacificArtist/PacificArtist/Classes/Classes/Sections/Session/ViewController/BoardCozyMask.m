...
//    }
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_team_getTeamSetting params:dict isShow:NO success:^(id responseObject) {
        @strongify(self)
...
//            }
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_team_getStatusSendText params:dict isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
    ...
    self.header = [[ThroughoutUniqueConcurrentAlcove alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, self.view.width, 0)];
...
- (void)onTapMenuItemMark:(OptimizeStencilForcefulPaintClose *)item
{
    ...
    [[NIMSDK sharedSDK].chatExtendManager addCollect:params completion:^(NSError * _Nullable error, NIMCollectInfo * _Nullable collectInfo) {
        if (error) {
            [ShapeSurfTerminalSystematic showErrorWithStatus:LangKey(@"收藏失败")];
            return;
        }
        [ShapeSurfTerminalSystematic showSuccessWithStatus:LangKey(@"收藏成功")];
    }];
}
...
- (void)onTapMenuItemTranslation:(OptimizeStencilForcefulPaintClose *)item
{
    ...
    [FertileSuiteEnableCacheLine getWithUrl:Server_getNimCheckSum params:param isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue <= 0) {
            NSDictionary *data = [resultDict valueObjectForKey:@"data"];
...
{
    if(!_translateView){
        _translateView = [[ScanFastSymbolFlat alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)didTouchSubmitContentButton:(NSString *)reason
{
    ...
    if(isMyFriend){
        ...
        NSMutableDictionary *dict = @{}.mutableCopy;
        dict[@"contact"] = reason;
        [FertileSuiteEnableCacheLine postWithUrl:Server_other_feedback params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_reprotNextView){
        _reprotNextView  = [[MapConsumeCollectorElevatedNextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotBlackView){
        _reprotBlackView  = [[MapConsumeCollectorElevatedBlackView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (MapConsumeCollectorElevatedDeleteView *)reprotDeleteView
{
    if(!_reprotDeleteView){
        _reprotDeleteView  = [[MapConsumeCollectorElevatedDeleteView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (MapConsumeCollectorElevatedView *)reprotHisNextView
{
    if(!_reprotHisNextView){
        _reprotHisNextView  = [[MapConsumeCollectorElevatedView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
