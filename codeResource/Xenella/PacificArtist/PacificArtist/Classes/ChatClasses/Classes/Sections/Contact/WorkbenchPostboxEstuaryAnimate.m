...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [ShapeSurfTerminalSystematic showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (RationalCreatorPastInsertTwist *)selectIndicatorView{
    ...
    _selectIndicatorView = [[RationalCreatorPastInsertTwist alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (ValueVerseFacade *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[ValueVerseFacade alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
