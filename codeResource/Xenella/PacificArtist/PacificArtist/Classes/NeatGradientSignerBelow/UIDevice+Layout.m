...
+ (CGFloat)barrelhouse {
    ...
    if (statusBarHeight <= 0) {
        ...
        CGFloat topPadding = [UIDevice sulkyStand];
...
+ (CGFloat)fullCoatCapital {
    //: return [UIDevice vg_statusBarHeight] + [UIDevice vg_navigationBarHeight];
    return [UIDevice barrelhouse] + [UIDevice lownessDepth];
}
...
+ (CGFloat)reading {
    ...
    return [UIDevice barrelhouse] + [UIDevice light];
}
