...
// __CLOSE_PRINT__
//
//  UINavigationBar+SelectorYearFeature.m
//  NIM
//
...
//: @implementation UINavigationBar (SelectorYearFeature)
@implementation UINavigationBar (SelectorYearFeature)
...
- (void)mode{
    ...
    CGPoint centerPonit = CGPointMake(self.discredit * .5f, self.exception *.5f);
...
- (void)factoryNumberro{
    //: [self swizzling_layoutSubviews];
    [self factoryNumberro];
    //: UINavigationItem *navigationItem = [self topItem];
    UINavigationItem *navigationItem = [self topItem];
    ...
    subview.video = navigationBtnMargin;
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(factoryNumberro));
    //: });
    });
}
