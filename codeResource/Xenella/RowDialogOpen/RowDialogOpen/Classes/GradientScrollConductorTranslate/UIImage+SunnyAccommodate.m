...
@implementation UIImage (SunnyAccommodate)
//: + (UIImage *)fetchImage:(NSString *)imageNameOrPath{
+ (UIImage *)house:(NSString *)imageNameOrPath{
...
- (UIImage *)outAdminUpload
{
    //: CGFloat pixels = [[USERDevice currentDevice] suggestImagePixels];
    CGFloat pixels = [[ThyDevice dataSuspend] listener];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self glassFront:pixels];
...
- (UIImage *)quantitativeRelationHolderMortal: (CGFloat)maxPixels
{
    ...
    return [self socialEvent:CGSizeMake(newSizeWidth, newSizeHeight)];
}
...
- (UIImage *)socialEvent:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
...
- (UIImage *)glassFront: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self quantitativeRelationHolderMortal:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self quantitativeRelationHolderMortal:suggestPixels];
    }
}
