...
- (void)setConditionMove:(NSAttributedString *)placeholderAttributedText
{
    //: _placeholderAttributedText = placeholderAttributedText;
    _conditionMove = placeholderAttributedText;
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSAttributedString *attributedString = self.conditionMove;
...
- (void)pieceReverse:(NSNotification *)notification
{
    //: [self updatePlaceholder];
    [self hour];
}
...
- (instancetype)initWithFrame:(CGRect)frame textContainer:(NSTextContainer *)textContainer
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pieceReverse:) name:UITextViewTextDidChangeNotification object:self];
    }
    //: return self;
    return self;
}
