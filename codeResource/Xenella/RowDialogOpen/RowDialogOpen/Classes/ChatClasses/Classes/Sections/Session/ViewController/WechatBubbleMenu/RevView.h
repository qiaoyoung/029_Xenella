...
- (void)betweenPrepare:(NSArray *)array
                 ...
                 pick:(CGRect)cursorStartRect object:(CGRect)rect automatic:(void(^)(YapAwayAgent *item))block
                     //: praiseBlock:(void(^)(NSInteger tag))praiseblock;
                     modelsBlock:(void(^)(NSInteger tag))praiseblock;
//: + (instancetype)shareMenuView;
+ (instancetype)hold;// 单例模式
