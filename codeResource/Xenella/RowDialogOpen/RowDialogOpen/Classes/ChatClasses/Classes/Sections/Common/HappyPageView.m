...
@implementation HappyPageView

//: - (NSInteger)pageInBound:(NSInteger)value min:(NSInteger)min max:(NSInteger)max{
- (NSInteger)takeAHop:(NSInteger)value list:(NSInteger)min advocateMarch:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)delegateLink
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(safely:cross:threadWith:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate safely:self
                                //: currentIndex:_currentPage
                                cross:_currentPage
                                  //: totolPages:[_pages count]];
                                  threadWith:[_pages count]];
    }
}
...
- (void)shot:(UIInterfaceOrientation)toInterfaceOrientation
                                ...
                                subsequently:(NSTimeInterval)duration
{
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(lessDeal)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate lessDeal];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(mannerBottomScroll:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate mannerBottomScroll:self];
    }
}
...
- (void)appear
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(themes:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource themes:self];
    }
...
- (void)appropriate:(NSInteger)currentPage
{
    ...
    NSInteger first = [self takeAHop:currentPage - 1 list:0 advocateMarch:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self takeAHop:currentPage + 1 list:0 advocateMarch:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(exceptConnectionPage:attribute:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource exceptConnectionPage:self attribute:index];
