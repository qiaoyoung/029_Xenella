...
+ (NSData *)StockDataToData:(NSString *)value {
...
//: @3x
+ (NSString *)colorSlipConfig {
    /* static */ NSString *colorSlipConfig = nil;
    if (!colorSlipConfig) {
		NSString *origin = @"14672cbe";
		NSData *data = [StockData StockDataToData:origin];
        StructStockData value = (StructStockData){84, (Byte *)data.bytes, 3, 117, 123};
        colorSlipConfig = [self StringFromStockData:&value];
    }
    return colorSlipConfig;
}

+ (NSString *)StringFromStockData:(StructStockData *)data {
    return [NSString stringWithUTF8String:(char *)[self StockDataToByte:data]];
}

//: @2x
+ (NSString *)colorStaffSettings {
    /* static */ NSString *colorStaffSettings = nil;
    if (!colorStaffSettings) {
		NSString *origin = @"6a185246";
		NSData *data = [StockData StockDataToData:origin];
        StructStockData value = (StructStockData){42, (Byte *)data.bytes, 3, 55, 16};
        colorStaffSettings = [self StringFromStockData:&value];
    }
    return colorStaffSettings;
}

//: default
+ (NSString *)appAccuracyHelper {
    /* static */ NSString *appAccuracyHelper = nil;
    if (!appAccuracyHelper) {
		NSString *origin = @"17161512061f07e0";
		NSData *data = [StockData StockDataToData:origin];
        StructStockData value = (StructStockData){115, (Byte *)data.bytes, 7, 194, 89};
        appAccuracyHelper = [self StringFromStockData:&value];
    }
    return appAccuracyHelper;
}

+ (Byte *)StockDataToByte:(StructStockData *)data {
    for (int i = 0; i < data->estate; i++) {
        data->modify[i] ^= data->excuseWrist;
    }
    data->modify[data->estate] = 0;
	if (data->estate >= 2) {
		data->lunchMinor = data->modify[0];
		data->heritage = data->modify[1];
	}
    return data->modify;
}

//: content
+ (NSString *)kAnticipatePreference {
    /* static */ NSString *kAnticipatePreference = nil;
    if (!kAnticipatePreference) {
		NSString *origin = @"272b2a30212a300a";
		NSData *data = [StockData StockDataToData:origin];
        StructStockData value = (StructStockData){68, (Byte *)data.bytes, 7, 156, 202};
        kAnticipatePreference = [self StringFromStockData:&value];
    }
    return kAnticipatePreference;
}

//: Chartlet
+ (NSString *)moduleDiscoveryShortsPlatform {
    /* static */ NSString *moduleDiscoveryShortsPlatform = nil;
    if (!moduleDiscoveryShortsPlatform) {
		NSString *origin = @"5f747d6e687079680c";
		NSData *data = [StockData StockDataToData:origin];
        StructStockData value = (StructStockData){28, (Byte *)data.bytes, 8, 37, 248};
        moduleDiscoveryShortsPlatform = [self StringFromStockData:&value];
    }
    return moduleDiscoveryShortsPlatform;
}
...
- (UIImage *)valueEven
{
    ...
    CGFloat pixels = [[AyMessage speedy] excessContrast];
    //: UIImage * image = [self nim_imageForUpload:pixels];
    UIImage * image = [self key:pixels];
    //: return [image nim_fixOrientation];
    return [image startingSafety];
}
...
- (UIImage *)holder: (CGFloat)maxPixels
{
    ...
    return [self opposition:CGSizeMake(newSizeWidth, newSizeHeight)];
}


//: - (UIImage *)nim_cropedImageWithSize:(CGSize)targetSize
- (UIImage *)streetSmart:(CGSize)targetSize
{
    // 裁剪两边
...
- (UIImage *)key: (CGFloat)suggestPixels
{
...
+ (UIImage *)shallow:(NSString *)imageName capacity:(NSString *)chartletId{
    ...
    if ([chartletId isEqualToString:[StockData appAccuracyHelper]]) {
        //: return [UIImage imageNamed:imageName];
        return [UIImage imageNamed:imageName];
    }
    //: NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",@"Chartlet",chartletId,@"content"];
    NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",[StockData moduleDiscoveryShortsPlatform],chartletId,[StockData kAnticipatePreference]];
    //先拿2倍图
    //: NSString *doubleImage = [imageName stringByAppendingString:@"@2x"];
    NSString *doubleImage = [imageName stringByAppendingString:[StockData colorStaffSettings]];
    //: NSString *tribleImage = [imageName stringByAppendingString:@"@3x"];
    NSString *tribleImage = [imageName stringByAppendingString:[StockData colorSlipConfig]];
...
- (UIImage *)opposition:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
