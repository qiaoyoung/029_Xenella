...
NSString *StringFromArchData(Byte *data);


//: content
Byte featureResEssenceSettings[] = {74, 7, 63, 14, 122, 43, 80, 39, 226, 83, 178, 13, 125, 195, 162, 174, 173, 179, 164, 173, 179, 10};

//: Chartlet
Byte widgetGapDevice[] = {60, 8, 34, 6, 66, 204, 101, 138, 131, 148, 150, 142, 135, 150, 34};

//: @2x
Byte themeClipFormat[] = {43, 3, 33, 4, 97, 83, 153, 242};

//: default
Byte k_practicalId[] = {44, 7, 61, 5, 199, 161, 162, 163, 158, 178, 169, 177, 148};

//: @3x
Byte coreSouTitle[] = {71, 3, 32, 13, 153, 202, 43, 124, 182, 218, 249, 168, 35, 96, 83, 152, 82};
...
@implementation UIImage (TaskIdentifyRave)
...
+ (UIImage *)containerWithoutHighlight:(NSString *)imageName random:(NSString *)chartletId{
    ...
    if ([chartletId isEqualToString:StringFromArchData(k_practicalId)]) {
        //: return [UIImage imageNamed:imageName];
        return [UIImage imageNamed:imageName];
    }
    //: NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",@"Chartlet",chartletId,@"content"];
    NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",StringFromArchData(widgetGapDevice),chartletId,StringFromArchData(featureResEssenceSettings)];
    //先拿2倍图
    //: NSString *doubleImage = [imageName stringByAppendingString:@"@2x"];
    NSString *doubleImage = [imageName stringByAppendingString:StringFromArchData(themeClipFormat)];
    //: NSString *tribleImage = [imageName stringByAppendingString:@"@3x"];
    NSString *tribleImage = [imageName stringByAppendingString:StringFromArchData(coreSouTitle)];
...
- (UIImage *)bringGroup: (CGSize)size
{
...
- (UIImage *)targetSize:(CGSize)newSize
{
...
- (UIImage *)force: (CGFloat)suggestPixels
{
        ...
        (width / height > maxRatio || height / width > maxRatio))
    {
        ...
        return [self threadMaker:maxPixels];
    }
    //: else
    else
    {
        //: return [self nim_scaleWithMaxPixels:suggestPixels];
        return [self threadMaker:suggestPixels];
    }
}

//: - (UIImage *)nim_scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)threadMaker: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self targetSize:CGSizeMake(newSizeWidth, newSizeHeight)];
}
...
Byte * ArchDataToCache(Byte *data) {
    int marketing = data[0];
    int poloEvident = data[1];
    Byte bite = data[2];
    int zoneButt = data[3];
    if (!marketing) return data + zoneButt;
    for (int i = zoneButt; i < zoneButt + poloEvident; i++) {
        int value = data[i] - bite;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[zoneButt + poloEvident] = 0;
    return data + zoneButt;
}

NSString *StringFromArchData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ArchDataToCache(data)];
}
