...
@implementation NSData (SunnyAccommodate)

//: - (NSData *)aes256EncryptWithKey:(NSString *)key vector:(NSString *)vector {
- (NSData *)comparative:(NSString *)key lab:(NSString *)vector {
...
void wholeSpeaker(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)scheme:(NSString *)key {
    ...
    wholeSpeaker(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
- (NSData *)localStreetWithItPassageTaproom:(NSString *)key spot:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
