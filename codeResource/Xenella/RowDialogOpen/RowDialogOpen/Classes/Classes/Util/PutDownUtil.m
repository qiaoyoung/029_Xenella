...
//: @implementation USERFileUtil
@implementation PutDownUtil

//: + (NSString *)fileMD5:(NSString *)filepath
+ (NSString *)reason:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self executeSize:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self rubberneck:filepath];
        //: }else {
        }else {
            //: NSData *data = [NSData dataWithContentsOfFile:filepath];
            NSData *data = [NSData dataWithContentsOfFile:filepath];
            //: md5 = [data MD5String];
            md5 = [data everyTransform];
        }
    }
    //: return md5;
    return md5;
...
+ (NSString *)rubberneck:(NSString *)filePath
{
...
+ (unsigned long long)executeSize:(NSString *)filepath
{
...
+ (void)draw:(NSString *)filepath hiddenWith:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self reason:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
