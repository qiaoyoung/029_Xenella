...
@implementation UIImage (CleanPro)
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(powers:));
...
+ (UIImage *)powers:(NSString *)name {
    ...
    UIImage *image = [[Control modernCouncil] powers:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self powers:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
