...
static char spacingVoiceUtility;


//: @implementation UIActionSheet (USERBlock)
@implementation UIActionSheet (Detail)


//: - (void)showInView: (UIView *)view completionHandler: (ActionSheetBlock)block
- (void)storm: (UIView *)view turn: (ActionSheetBlock)block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&spacingVoiceUtility,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        UITabBar *tabbar = [self conceptCrop];
        //: if (tabbar)
        if (tabbar)
        {
        ...
        else
        {
            ...
            [self performSelector:@selector(ignoreMotion:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &spacingVoiceUtility) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&spacingVoiceUtility,nil,OBJC_ASSOCIATION_COPY);
