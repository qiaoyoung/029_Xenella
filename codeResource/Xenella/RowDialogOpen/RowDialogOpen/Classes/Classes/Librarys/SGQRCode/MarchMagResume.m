...
@interface MarchMagResume ()
{
    //: SystemSoundID _soundID;
    SystemSoundID _accumulation;
}
//: @end
@end

//: @implementation SGSoundEffect
@implementation MarchMagResume

//: - (void)dealloc {
- (void)dealloc {
    //: AudioServicesDisposeSystemSoundID(_soundID);
    AudioServicesDisposeSystemSoundID(_accumulation);
}

//: - (id)initWithFilepath:(NSString *)path {
- (id)initWithHold:(NSString *)path {
    //: self = [super init];
    self = [super init];
...
+ (id)nationFilepath:(NSString *)path {
    //: if (path) {
    if (path) {
        //: return [[SGSoundEffect alloc] initWithFilepath:path];
        return [[MarchMagResume alloc] initWithHold:path];
    }
    //: return nil;
    return nil;
}
...
- (void)woman {
    ...
    AudioServicesPlaySystemSound(_accumulation);
}
