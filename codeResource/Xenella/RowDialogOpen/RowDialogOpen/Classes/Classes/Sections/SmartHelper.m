...
//: colors
- (NSString *)componentGooDistributeKey {
    /* static */ NSString *componentGooDistributeKey = nil;
    if (!componentGooDistributeKey) {
        StructMoralData value = (StructMoralData){176, (Byte []){211, 223, 220, 223, 194, 195, 229}, 6, 3};
        componentGooDistributeKey = [self StringFromMoralData:&value];
    }
    return componentGooDistributeKey;
}

//: chromateAnimate
- (NSString *)styleBuildFormat {
    /* static */ NSString *styleBuildFormat = nil;
    if (!styleBuildFormat) {
        StructMoralData value = (StructMoralData){169, (Byte []){202, 193, 219, 198, 196, 200, 221, 204, 232, 199, 192, 196, 200, 221, 204, 22}, 15, 13};
        styleBuildFormat = [self StringFromMoralData:&value];
    }
    return styleBuildFormat;
}

+ (instancetype)sharedInstance {
    static MoralData *instance = nil;
...
- (Byte *)MoralDataToByte:(StructMoralData *)data {
    for (int i = 0; i < data->determinate; i++) {
        data->he[i] ^= data->rescue;
    }
    data->he[data->determinate] = 0;
	if (data->determinate >= 1) {
		data->reamGrowing = data->he[0];
	}
    return data->he;
}
...
+ (UIImage *)heritageRecent:(UIColor *)centerColor emotionSelect:(UIColor *)outColor {
    ...
    return [self gradientPresent:centerColor piece:outColor tapRepresentationOption:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice theoretical]))];
}
...
+ (void)enter:(CGContextRef)context
                      //: path:(CGPathRef)path
                      council:(CGPathRef)path
                //: startColor:(CGColorRef)startColor
                escape:(CGColorRef)startColor
                  //: endColor:(CGColorRef)endColor {
                  boot:(CGColorRef)endColor {
...
+ (void)weltAcross:(UIView *)parentView eliteTo_strong:(UILabel *)lable {
    ...
    CAKeyframeAnimation *chromateAnimate = [self corner];

    //: [chromatoLayer addAnimation:chromateAnimate forKey:@"chromateAnimate"];
    [chromatoLayer addAnimation:chromateAnimate forKey:[[MoralData sharedInstance] styleBuildFormat]];
...
+ (CAKeyframeAnimation *)corner {
    //: CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:@"colors"];
    CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:[[MoralData sharedInstance] componentGooDistributeKey]];
...
+ (UIImage *)failure:(UIColor *)startColor canadianProvinceType:(UIColor *)endColor outMeasureZone:(SNGradientDirection)directionType {
    ...
    return [self slight:startColor vendor:endColor timingOption:directionType measure:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice theoretical]))];
}
...
+ (UIImage *)gradientPresent:(UIColor *)centerColor piece:(UIColor *)outColor tapRepresentationOption:(CGSize)size {
    ...
    [self enter:gc council:path escape:centerColor.CGColor boot:outColor.CGColor];
    //: CGPathRelease(path);
    CGPathRelease(path);
