...
@implementation UINavigationBar (Decent)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(startAlbum));
    //: });
    });
}
...
- (void)startAlbum{
    ...
    subview.air = navigationBtnMargin;
...
- (void)partFail{
    ...
    CGPoint centerPonit = CGPointMake(self.capability * .5f, self.year *.5f);
