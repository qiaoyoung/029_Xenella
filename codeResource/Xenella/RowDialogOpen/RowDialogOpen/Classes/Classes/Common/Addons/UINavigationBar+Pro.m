...
- (void)disabledGrowing{
    //: [self swizzling_layoutSubviews];
    [self disabledGrowing];
    ...
    subview.technology = navigationBtnMargin;
...
- (void)theoretical{
    ...
    CGPoint centerPonit = CGPointMake(self.system * .5f, self.transshipmentCenter *.5f);
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(disabledGrowing));
    //: });
    });
}
