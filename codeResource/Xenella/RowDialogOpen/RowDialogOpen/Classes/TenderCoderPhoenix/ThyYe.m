...
@interface ThyYe ()
{
    ...
    SystemSoundID _cancel;
}
//: @end
@end

//: @implementation SGSoundEffect
@implementation ThyYe

//: - (void)play {
- (void)sample {
    //: AudioServicesPlaySystemSound(_soundID);
    AudioServicesPlaySystemSound(_cancel);
}

//: - (id)initWithFilepath:(NSString *)path {
- (id)initWithThat:(NSString *)path {
    //: self = [super init];
    self = [super init];
...
- (void)dealloc {
    ...
    AudioServicesDisposeSystemSoundID(_cancel);
}

//: + (id)soundEffectWithFilepath:(NSString *)path {
+ (id)cancel:(NSString *)path {
    //: if (path) {
    if (path) {
        //: return [[SGSoundEffect alloc] initWithFilepath:path];
        return [[ThyYe alloc] initWithThat:path];
    }
    //: return nil;
    return nil;
}
