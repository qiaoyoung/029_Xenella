...
@interface OftLargeView ()

//: @property (strong) UIColor *badgeTextColor;
@property (strong) UIColor *wakeful;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor clearColor];
	[self setWakeful:_badgeTextColor];
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self message:badgeValue];
	[self setWakeful:_badgeTextColor];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self overContext:rect byContextRef:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self complete:rect intervalerlingualRendition:context];
    }
...
- (void)overContext:(CGRect)rect byContextRef:(CGContextRef)context{
...
- (CGRect)message:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self formation:badgeValue];
...
+ (instancetype)dragStatement:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    ...
    OftLargeView *instance = [[OftLargeView alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance message:badgeValue];
...
- (void)setWakeful:(UIColor *)wakeful {
...
- (void)complete:(CGRect)rect intervalerlingualRendition:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
...
- (CGSize)formation:(NSString *)badgeValue{
