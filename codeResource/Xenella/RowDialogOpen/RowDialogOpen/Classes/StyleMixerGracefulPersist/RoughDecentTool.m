...
@implementation RoughDecentTool

//: #pragma mark - callback
#pragma mark - callback
//: + (void)executeCallback:(void (^)(EnumAuthorizationStatus))callback status:(EnumAuthorizationStatus)status {
+ (void)aboveRave:(void (^)(EnumAuthorizationStatus))callback nether:(EnumAuthorizationStatus)status {
...
+ (void)determinateOpera:(void(^)(EnumAuthorizationStatus status))callback
{
        ...
        if (authStatus == ALAuthorizationStatusNotDetermined) { // 未授权
            ...
            [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
                ...
                if (status == PHAuthorizationStatusAuthorized) {
                    ...
                    [self aboveRave:callback nether:EnumAuthorizationStatusAuthorized];
                //: } else if (status == PHAuthorizationStatusDenied) {
                } else if (status == PHAuthorizationStatusDenied) {
                    //: [self executeCallback:callback status:EnumAuthorizationStatusDenied];
                    [self aboveRave:callback nether:EnumAuthorizationStatusDenied];
                //: } else if (status == PHAuthorizationStatusRestricted) {
                } else if (status == PHAuthorizationStatusRestricted) {
                    //: [self executeCallback:callback status:EnumAuthorizationStatusRestricted];
                    [self aboveRave:callback nether:EnumAuthorizationStatusRestricted];
                }
            //: }];
            }];
        }
    ...
    //: } else {
        ...
        [self aboveRave:callback nether:EnumAuthorizationStatusNotSupport];
    }
}
