...
//: default
- (NSString *)spacingLiverPage {
    /* static */ NSString *spacingLiverPage = nil;
    if (!spacingLiverPage) {
		NSArray<NSString *> *origin = @[@"7", @"74", @"11", @"100", @"39", @"51", @"148", @"48", @"85", @"125", @"179", @"26", @"27", @"28", @"23", @"43", @"34", @"42", @"54"];
		NSData *data = [DumpData DumpDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        spacingLiverPage = [self StringFromDumpData:value];
    }
    return spacingLiverPage;
}

+ (NSData *)DumpDataToData:(NSArray<NSString *> *)value {
...
- (Byte *)DumpDataToCache:(Byte *)data {
    int phrase = data[0];
    Byte nearEdge = data[1];
    int twin = data[2];
    for (int i = twin; i < twin + phrase; i++) {
        int value = data[i] + nearEdge;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
...
//: Chartlet
- (NSString *)styleAgileIllegalNecessarilyId {
    /* static */ NSString *styleAgileIllegalNecessarilyId = nil;
    if (!styleAgileIllegalNecessarilyId) {
		NSArray<NSString *> *origin = @[@"8", @"77", @"8", @"80", @"222", @"223", @"52", @"142", @"246", @"27", @"20", @"37", @"39", @"31", @"24", @"39", @"162"];
		NSData *data = [DumpData DumpDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        styleAgileIllegalNecessarilyId = [self StringFromDumpData:value];
    }
    return styleAgileIllegalNecessarilyId;
}
...
//: content
- (NSString *)moduleBecauseTimer {
    /* static */ NSString *moduleBecauseTimer = nil;
    if (!moduleBecauseTimer) {
		NSArray<NSString *> *origin = @[@"7", @"57", @"13", @"251", @"61", @"156", @"39", @"81", @"71", @"233", @"170", @"238", @"204", @"42", @"54", @"53", @"59", @"44", @"53", @"59", @"64"];
		NSData *data = [DumpData DumpDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        moduleBecauseTimer = [self StringFromDumpData:value];
    }
    return moduleBecauseTimer;
}
...
//: @implementation UIImage (ParseByBreakPerform)
@implementation UIImage (ParseByBreakPerform)

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)nim_scaleToSize:(CGSize)newSize
- (UIImage *)size:(CGSize)newSize
{
    ...
    return [self scour:size];
}


//: - (UIImage *)nim_cropedImageWithSize:(CGSize)targetSize
- (UIImage *)untilSize:(CGSize)targetSize
{
    // 裁剪两边
...
{
    //: CGFloat pixels = [[PipelineLucidStripeWeaveAttach currentDevice] suggestImagePixels];
    CGFloat pixels = [[PipelineLucidStripeWeaveAttach resign] graph];
    //: UIImage * image = [self nim_imageForUpload:pixels];
    UIImage * image = [self unwishedLess:pixels];
...
- (UIImage *)absolutePixels: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self size:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//: + (UIImage *)nim_fetchChartlet:(NSString *)imageName chartletId:(NSString *)chartletId{
+ (UIImage *)exit:(NSString *)imageName receive:(NSString *)chartletId{
    //: if ([chartletId isEqualToString:@"default"]) {
    if ([chartletId isEqualToString:[[DumpData sharedInstance] spacingLiverPage]]) {
        //: return [UIImage imageNamed:imageName];
        return [UIImage imageNamed:imageName];
    }
    //: NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",@"Chartlet",chartletId,@"content"];
    NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",[[DumpData sharedInstance] styleAgileIllegalNecessarilyId],chartletId,[[DumpData sharedInstance] moduleBecauseTimer]];
    //先拿2倍图
...
- (UIImage *)unwishedLess: (CGFloat)suggestPixels
{
        ...
        (width / height > maxRatio || height / width > maxRatio))
    {
        ...
        return [self absolutePixels:maxPixels];
    }
    //: else
    else
    {
        //: return [self nim_scaleWithMaxPixels:suggestPixels];
        return [self absolutePixels:suggestPixels];
    }
}
