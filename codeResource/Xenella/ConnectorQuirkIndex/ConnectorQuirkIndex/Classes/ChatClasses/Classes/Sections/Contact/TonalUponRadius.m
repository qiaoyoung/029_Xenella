...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [ResizeNucleusDocumentRow showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (RecordBrokerRealm *)selectIndicatorView{
    ...
    _selectIndicatorView = [[RecordBrokerRealm alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (ToRoundOriginalHeavenFairy *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[ToRoundOriginalHeavenFairy alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
