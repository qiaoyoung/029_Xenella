...
NSString *StringFromLeapData(Byte *data);


//: FFFKitEventNameTapContent
Byte k_orientationId[] = {61, 25, 3, 4, 73, 73, 73, 78, 108, 119, 72, 121, 104, 113, 119, 81, 100, 112, 104, 87, 100, 115, 70, 114, 113, 119, 104, 113, 119, 32};

// __DEBUG__
// __CLOSE_PRINT__
//
//  AggregateConverterThornEnd.m
// ParseByBreakPerform
//
...
//: @interface AggregateConverterThornEnd()
@interface AggregateConverterThornEnd()

//: @property (nonatomic,strong,readwrite) YYAnimatedImageView * imageView;
@property (nonatomic,strong,readwrite) YYAnimatedImageView * consistent;
@property (nonatomic,strong,readwrite) YYAnimatedImageView * imageView;
...
//: @implementation AggregateConverterThornEnd
@implementation AggregateConverterThornEnd

//: @end

- (void)setConsistent:(YYAnimatedImageView *)consistent {
    //: OC_CUSTOM_PROPERTY_INJECT
    _consistent = consistent;
}

- (YYAnimatedImageView *)afterUnchanging:(YYAnimatedImageView *)consistent {
    //: OC_CUSTOM_PROPERTY_INJECT
    _consistent = consistent;
    return consistent;
}
...
- (void)asGrace:(id)sender
{
    ...
    event.woods = StringFromLeapData(k_orientationId);
	[self setConsistent:_imageView];
...
- (instancetype)initFinish{
    ...
    if (self) {
        ...
        [self afterUnchanging:_imageView].backgroundColor = [UIColor clearColor];
        //: _imageView.contentMode = UIViewContentModeScaleAspectFill;
        [self afterUnchanging:_imageView].contentMode = UIViewContentModeScaleAspectFill;
...
- (void)cycle:(MysticSnowColorfulComposer *)data
{
    ...
    NIMImageObject * imageObject = (NIMImageObject*)self.publishColorfulComposer.across.messageObject;
    ...
    if (imageObject.url.length > 0) {
        ...
        NSData *data = [dic objectForKey:self.publishColorfulComposer.across.messageId];
...
//        [_imageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:images ];
        ...
        [[self afterUnchanging:_imageView] sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:images options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
    ...
    //: } else if (imageObject.thumbPath.length > 0) {
        ...
        if (image != nil) {
            ...
            [self afterUnchanging:_imageView].image = image;
        }
    //: } else {
    } else {
    ...
    if (self.publishColorfulComposer.across.deliveryState == NIMMessageDeliveryStateDelivering) {
        //: self.progressView.hidden = NO;
        self.progressView.hidden = NO;
        //: self.progressView.progress = [[NIMSDK sharedSDK].chatManager messageTransportProgress:self.model.message];
        self.progressView.advancement = [[NIMSDK sharedSDK].chatManager messageTransportProgress:self.publishColorfulComposer.across];
    //: }else{
    }else{
        //: self.progressView.hidden = YES;
        self.progressView.hidden = YES;
...
- (void)layoutSubviews{
    ...
    [self afterUnchanging:self.imageView].frame = imageViewFrame;
    ...
    maskLayer.frame = [self afterUnchanging:self.imageView].bounds;
...
Byte * LeapDataToCache(Byte *data) {
    int mediumPone = data[0];
    int sameKnown = data[1];
    Byte oy = data[2];
    int loser = data[3];
    if (!mediumPone) return data + loser;
    for (int i = loser; i < loser + sameKnown; i++) {
        int value = data[i] - oy;
        if (value < 0) {
            value += 256;
        }
    ...
    data[0] = 0;
    data[loser + sameKnown] = 0;
    return data + loser;
}

NSString *StringFromLeapData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)LeapDataToCache(data)];
}
