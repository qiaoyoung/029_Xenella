...
- (void)setupSecureBackground {
    ...
    bgView.backgroundColor = RGB_COLOR_String(@"F6F7FA");
    if (!bgView) {
...
- (void)restoreOriginalView {
    ...
    originalView.backgroundColor = RGB_COLOR_String(@"F6F7FA");
    // 将当前视图的所有子视图转移到新视图
...
{
    if(!_customAlbumView){
        _customAlbumView = [[OutlineImportAnalyzeReset alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_audioContent){
        _audioContent = [[DeriveContent alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
-(UITextField *)bgTextField{
    if (!_bgTextField) {
        ...
        _bgTextField.backgroundColor = RGB_COLOR_String(@"F6F7FA");
    }
    return _bgTextField;
}
