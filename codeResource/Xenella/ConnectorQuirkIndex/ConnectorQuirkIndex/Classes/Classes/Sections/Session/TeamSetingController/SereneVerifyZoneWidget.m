...
- (void)updateTeamName{
    ...
    [self.groupnameView animationShow];
        ...
        self.groupnameView.speiceBackBlock = ^(NSString *groupName){
            ...
            [LocalizeHelixAccept getWithUrl:Server_team_modifyname params:dict isShow:NO success:^(id responseObject) {
                [self reloadData];
...
- (void)dismissTeam {
    ...
    [self.groupAlartView animationShow];
        ...
        self.groupAlartView.speiceBackBlock = ^(NSString *Name){
            @strongify(self)
            [self didDismissTeam];
            [LocalizeHelixAccept getWithUrl:Server_team_destory params:@{@"id":self.teamListManager.team.teamId} isShow:NO success:^(id responseObject) {
...
- (void)didOntransferWithLeave:(BOOL)isLeave {
    ...
    __block ContactSelectFinishBlock finishBlock =  ^(NSArray * memeber, NSString *name, UIImage *avater){
        ...
        [LocalizeHelixAccept getWithUrl:Server_team_modifyowner params:dict isShow:NO success:^(id responseObject) {
            [wself reloadData];
        } failed:^(id responseObject, NSError *error) {
...
- (BufferPainterShuffle *)aleartView{
    if(!_aleartView){
        _aleartView = [[BufferPainterShuffle alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_groupnameView){
        _groupnameView = [[PleasantLayoutMeritStrategy alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (GatewayClip *)groupNickNameView{
    if(!_groupNickNameView){
        _groupNickNameView = [[GatewayClip alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
