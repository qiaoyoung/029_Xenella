...
// __CLOSE_PRINT__
//
//  NSData+ClassicDetailAfterEvent.m
//  NIM
//
...
void stackPhase(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)eliminationReaction:(NSString *)key {
    ...
    stackPhase(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
- (NSData *)indicator:(NSString *)key permissionVector:(NSString *)vector {
...
- (NSData *)with:(NSString *)key over:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
