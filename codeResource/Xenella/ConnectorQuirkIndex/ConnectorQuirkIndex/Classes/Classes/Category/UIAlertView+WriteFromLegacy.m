...
static char layoutDisableAlert;

//: @implementation UIAlertView (WriteFromLegacy)
@implementation UIAlertView (WriteFromLegacy)
...
- (void)judgeBlock
{
    ...
    objc_setAssociatedObject(self, &layoutDisableAlert, nil, OBJC_ASSOCIATION_COPY);
}


//: - (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
- (void)send:(UIAlertView *)alertView independence:(NSInteger)buttonIndex
{
    //: AlertBlock block = objc_getAssociatedObject(self, &kUIAlertViewBlockAddress);
    AlertBlock block = objc_getAssociatedObject(self, &layoutDisableAlert);
    //: if (block)
    if (block)
    {
        //: block(buttonIndex);
...
- (void)under: (void (^)(NSInteger))block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIAlertViewBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&layoutDisableAlert,block,OBJC_ASSOCIATION_COPY);
    //: [self show];
    [self show];
}
...
//: @implementation UIAlertController (WriteFromLegacy)
@implementation UIAlertController (WriteFromLegacy)
