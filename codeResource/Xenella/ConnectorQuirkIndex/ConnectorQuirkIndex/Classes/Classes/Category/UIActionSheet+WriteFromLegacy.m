...
// __CLOSE_PRINT__
//
//  UIActionSheet+WriteFromLegacy.m
//  eim_iphone
//
...
static char screenLimitedContent;


//: @implementation UIActionSheet (WriteFromLegacy)
@implementation UIActionSheet (WriteFromLegacy)
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &screenLimitedContent) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&screenLimitedContent,nil,OBJC_ASSOCIATION_COPY);
...
- (void)scene: (UIView *)view indicator: (ActionSheetBlock)block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&screenLimitedContent,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        else
        {
            ...
            [self performSelector:@selector(sumensified:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
