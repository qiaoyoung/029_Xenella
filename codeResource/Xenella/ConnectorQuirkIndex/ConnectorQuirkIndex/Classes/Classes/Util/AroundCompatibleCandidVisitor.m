...
// __CLOSE_PRINT__
//
//  AroundCompatibleCandidVisitor.m
//  NIM
//
...
//: @implementation AroundCompatibleCandidVisitor
@implementation AroundCompatibleCandidVisitor

//: + (NSString *)bigFileMD5:(NSString *)filePath
+ (NSString *)sector:(NSString *)filePath
{
...
+ (NSString *)green:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self sage:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self sector:filepath];
        //: }else {
        }else {
...
+ (void)dropItem:(NSString *)filepath strong:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self green:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (unsigned long long)sage:(NSString *)filepath
{
