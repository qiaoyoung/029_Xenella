//
//  AAAA_USERVideoViewController_BBBB.m
//  NIM
//
...
//#import "WMZDialog.h"
@interface AAAA_USERVideoViewController_BBBB ()<AAAA_LYVideoPlayerDelegate_BBBB>

@property (nonatomic,strong) AAAA_USERVideoViewItem_BBBB *item;
@property (nonatomic,strong) AAAA_USERAVMoviePlay_BBBBController *avPlayer;
...
@property (nonatomic, strong) AAAA_LYSlider_BBBB *videoSlider;  //滑动条
...
@implementation AAAA_USERVideoViewController_BBBB
{
...
//    if (_avPlayer.playbackState == AAAA_USERAVMoviePlay_BBBBbackStatePlaying) {//不要调用.get方法，会过早的初始化播放器
...
//        if (wself)
//        {
//            [AAAA_SVProgressHUD_BBBB showProgress:progress];
//        }
...
//    self.avPlayer.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(moviePlaybackComplete:)
                                                 name:@"AAAA_USERAVMoviePlay_BBBBerPlaybackDidFinishNotification"
                                               object:self.avPlayer];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(moviePlayStateChanged:)
                                                 name:@"AAAA_USERAVMoviePlay_BBBBerPlaybackStateDidChangeNotification"
                                               object:self.avPlayer];
...
- (void)moviePlaybackComplete: (NSNotification *)aNotification
{
    if (self.avPlayer == aNotification.object)
    {
        ...
        NSNumber *resultValue = [notificationUserInfo objectForKey:@"AAAA_USERAVMoviePlay_BBBBerPlaybackDidFinishReasonUserInfoKey"];
        AAAA_USERAVMoviePlay_BBBBFinishReason reason = [resultValue intValue];
        if (reason == AAAA_USERAVMoviePlay_BBBBFinishReasonPlaybackError)
        {
...
- (void)moviePlayStateChanged: (NSNotification *)aNotification
{
    if (self.avPlayer == aNotification.object)
    {
        ...
        {
            case AAAA_USERAVMoviePlay_BBBBbackStatePlaying:
                [self topStatusUIHidden:YES];
                break;
            case AAAA_USERAVMoviePlay_BBBBbackStatePaused:
            case AAAA_USERAVMoviePlay_BBBBbackStateStopped:
            case AAAA_USERAVMoviePlay_BBBBbackStateInterrupted:
                [self topStatusUIHidden:NO];
            case AAAA_USERAVMoviePlay_BBBBbackStateSeekingBackward:
            case AAAA_USERAVMoviePlay_BBBBbackStateSeekingForward:
                break;
        }
...
- (AAAA_USERAVMoviePlay_BBBBController *)avPlayer {
    if (!_avPlayer) {
        _avPlayer = [[AAAA_USERAVMoviePlay_BBBBController alloc] initWithContentURL:[NSURL fileURLWithPath:self.item.path]];
        _avPlayer.delegate = self;
        _avPlayer.scalingMode = AAAA_USERAVMoviePlay_BBBBScalingModeAspectFill;
    }
    return _avPlayer;
}
...
- (AAAA_LYSlider_BBBB *)videoSlider{
    if (!_videoSlider) {
        _videoSlider = [[AAAA_LYSlider_BBBB alloc] initWithFrame:CGRectMake(0, 0, 3*(SCREEN_WIDTH/4), 20)];
...
{
    if(!_loadingView){
        _loadingView = [[AAAA_ZMONCustomLoadingView_BBBB alloc]initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT-SCREEN_TOP_HEIGHT-SCREEN_TABBAR_HEIGHT)];
    }
    return  _loadingView;
}
...
@implementation AAAA_USERVideoViewItem_BBBB
@end
