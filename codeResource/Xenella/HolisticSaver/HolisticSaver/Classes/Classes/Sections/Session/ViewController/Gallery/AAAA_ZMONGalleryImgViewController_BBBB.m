//
//  AAAA_ZMONGalleryImgViewController_BBBB.m
//  NIM
//
...
@implementation AAAA_USERGalleryItem_BBBB

@end

@interface AAAA_ZMONGalleryImgViewController_BBBB ()
...
//AAAA_TKImageView_BBBB
@property (nonatomic, strong) AAAA_TKImageView_BBBB *tkImageView;
//
@property (nonatomic, strong) AAAA_ZMONCustomLoadingView_BBBB *loadingView;
@end

@implementation AAAA_ZMONGalleryImgViewController_BBBB
...
//    }else{
        UIImage *image = [self imageWithPath:self.imagePath];
        [AAAA_FFFKitAuthorizationTool_BBBB requestPhotoLibraryAuthorization:^(AAAA_EnumAuthorizationStatus_BBBB status) {
            switch (status) {
                case AAAA_EnumAuthorizationStatus_BBBBAuthorized:
                    UIImageWriteToSavedPhotosAlbum(image, self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
                    break;
                default:
...
- (AAAA_TKImageView_BBBB *)tkImageView
{
    if(!_tkImageView){
        _tkImageView = [[AAAA_TKImageView_BBBB alloc]initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT-SCREEN_TOP_HEIGHT-UITabbarHeight)];
    }
    return _tkImageView;
}
