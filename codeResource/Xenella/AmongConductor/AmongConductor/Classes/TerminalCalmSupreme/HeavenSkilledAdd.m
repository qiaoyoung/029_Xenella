...
//: @implementation HeavenSkilledAdd
@implementation HeavenSkilledAdd
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self numberyStr:badgeValue];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self latissimusDorsi:rect associate:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self physicalProperty:rect basicContextRef:context];
    }
...
- (CGSize)databaseStr:(NSString *)badgeValue{
...
+ (instancetype)draw:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    }
    //: HeavenSkilledAdd *instance = [[HeavenSkilledAdd alloc] init];
    HeavenSkilledAdd *instance = [[HeavenSkilledAdd alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance numberyStr:badgeValue];
...
- (CGRect)numberyStr:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self databaseStr:badgeValue];
...
- (void)physicalProperty:(CGRect)rect basicContextRef:(CGContextRef)context{
...
- (void)latissimusDorsi:(CGRect)rect associate:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
