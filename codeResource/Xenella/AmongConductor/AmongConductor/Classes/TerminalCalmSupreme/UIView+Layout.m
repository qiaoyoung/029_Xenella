...
NSString *StringFromStressorData(Byte *data);        


//: bounds
Byte themeAdminPath[] = {77, 6, 7, 8, 55, 3, 32, 58, 91, 104, 110, 103, 93, 108, 183};

//: groupAnimationAlert
Byte colorGroupFormat[] = {65, 19, 17, 11, 181, 240, 31, 228, 100, 176, 82, 86, 97, 94, 100, 95, 48, 93, 88, 92, 80, 99, 88, 94, 93, 48, 91, 84, 97, 99, 1};

//: position
Byte commonBalanceStockData[] = {76, 8, 91, 9, 134, 88, 98, 110, 145, 21, 20, 24, 14, 25, 14, 20, 19, 238};

//: groupAnimationHide
Byte componentWeatherFormat[] = {6, 18, 11, 8, 221, 175, 210, 120, 92, 103, 100, 106, 101, 54, 99, 94, 98, 86, 105, 94, 100, 99, 61, 94, 89, 90, 57};
...
static char themeMagnituderoduceName; //被Present的View
//: static char PresentingViewAddress; 
static char componentMindDestinationValue; //正在Present其他视图的view

//: - (void)presentView:(UIView*)view animated:(BOOL)animated complete:(void(^)(void)) complete{
- (void)fastBreaking:(UIView*)view vanguardSecond:(BOOL)animated gray:(void(^)(void)) complete{
    //: if (!self.window) {
    if (!self.window) {
        //: return;
        return;
    ...
    objc_setAssociatedObject(self, &themeMagnituderoduceName, view, OBJC_ASSOCIATION_RETAIN);
    //: objc_setAssociatedObject(view, &PresentingViewAddress, self, OBJC_ASSOCIATION_RETAIN);
    objc_setAssociatedObject(view, &componentMindDestinationValue, self, OBJC_ASSOCIATION_RETAIN);
    //: if (animated) {
    if (animated) {
        //: [self doAlertAnimate:view complete:complete];
        [self circle:view user:complete];
    //: }else{
    }else{
...
- (void)arrow:(UIView*)alertView resistance:(void(^)(void)) complete{
    //: if (!alertView) {
    if (!alertView) {
        //: return;
        return;
    ...
    CABasicAnimation *scaleAnimation = [CABasicAnimation animationWithKeyPath:StringFromStressorData(themeAdminPath)];
    ...
    CABasicAnimation *moveAnimation = [CABasicAnimation animationWithKeyPath:StringFromStressorData(commonBalanceStockData)];
    ...
    [self until:alertView];
    //: alertView.backgroundColor = [UIColor clearColor];
    alertView.backgroundColor = [UIColor clearColor];

    //: [alertView.layer addAnimation:group forKey:@"groupAnimationHide"];
    [alertView.layer addAnimation:group forKey:StringFromStressorData(componentWeatherFormat)];
    ...
    dispatch_after(dispatch_time((0ull), (int64_t)(.25f * 1000000000ull)), dispatch_get_main_queue(), ^{
        ...
        [wself unsuitablenessObject];
        //: [wself showAllSubView:alertView];
        [wself voice:alertView];
        //: if (complete) {
        if (complete) {
            //: complete();
            complete();
...
- (void)onUntil:(id)sender{
    //: [self dismissPresentedView:YES complete:nil];
    [self collect:YES completeTo:nil];
}

//: - (void)showAllSubView:(UIView*)view{
- (void)voice:(UIView*)view{
    //: NSMutableArray *array = objc_getAssociatedObject(self,&HideViewsAddress);
    NSMutableArray *array = objc_getAssociatedObject(self,&widgetProceedData);
...
- (void)collect:(BOOL)animated completeTo:(void(^)(void)) complete{
    //: UIView * view = objc_getAssociatedObject(self, &PresentedViewAddress);
    UIView * view = objc_getAssociatedObject(self, &themeMagnituderoduceName);
    //: if (animated) {
    if (animated) {
        //: [self doHideAnimate:view complete:complete];
        [self arrow:view resistance:complete];
    //: }else{
    }else{
        //: [view removeFromSuperview];
        [view removeFromSuperview];
        //: [self cleanAssocaiteObject];
        [self unsuitablenessObject];
    }
}

//: - (void)cleanAssocaiteObject{
- (void)unsuitablenessObject{
    //: objc_setAssociatedObject(self,&PresentedViewAddress,nil,OBJC_ASSOCIATION_RETAIN);
    objc_setAssociatedObject(self,&themeMagnituderoduceName,nil,OBJC_ASSOCIATION_RETAIN);
    //: objc_setAssociatedObject(self,&PresentingViewAddress,nil,OBJC_ASSOCIATION_RETAIN);
    objc_setAssociatedObject(self,&componentMindDestinationValue,nil,OBJC_ASSOCIATION_RETAIN);
    //: objc_setAssociatedObject(self,&HideViewsAddress,nil, OBJC_ASSOCIATION_RETAIN);
    objc_setAssociatedObject(self,&widgetProceedData,nil, OBJC_ASSOCIATION_RETAIN);
}
...
- (void)lowAcross:(BOOL)animated ring:(void(^)(void)) complete{
    //: UIView * baseView = objc_getAssociatedObject(self, &PresentingViewAddress);
    UIView * baseView = objc_getAssociatedObject(self, &componentMindDestinationValue);
    //: if (!baseView) {
    if (!baseView) {
        //: return;
        return;
    ...
    [baseView collect:animated completeTo:complete];
    //: [self cleanAssocaiteObject];
    [self unsuitablenessObject];
}

//: static char *HideViewsAddress = "hideViewsAddress";
static char *widgetProceedData = "hideViewsAddress";
//: - (void)hideAllSubView:(UIView*)view{
- (void)until:(UIView*)view{
    //: for (UIView * subView in view.subviews) {
    for (UIView * subView in view.subviews) {
        ...
        objc_setAssociatedObject(self, &widgetProceedData, array, OBJC_ASSOCIATION_RETAIN);
        //: subView.hidden = YES;
        subView.hidden = YES;
    }
}
...
- (void)circle:(UIView*)view user:(void(^)(void)) complete{
    ...
    [view.layer addAnimation:group forKey:StringFromStressorData(colorGroupFormat)];
...
Byte * StressorDataToCache(Byte *data) {
    int biddingLick = data[0];
    int circleOfCurvature = data[1];
    Byte campaignBeneath = data[2];
    int good = data[3];
    if (!biddingLick) return data + good;
    for (int i = good; i < good + circleOfCurvature; i++) {
        int value = data[i] + campaignBeneath;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[good + circleOfCurvature] = 0;
    return data + good;
}

NSString *StringFromStressorData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)StressorDataToCache(data)];
}
