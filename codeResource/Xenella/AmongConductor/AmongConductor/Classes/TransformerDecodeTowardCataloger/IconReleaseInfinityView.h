 ...
 *  菜单展现的动画类型
 */
//: typedef NS_ENUM(NSInteger, IconReleaseInfinityViewAnimateType) {
typedef NS_ENUM(NSInteger, IconReleaseInfinityViewAnimateType) {
    /**
     *  scale based the point of upper right corner.
     *  以右上角为基点进行伸缩
     */
    //: IconReleaseInfinityViewAnimateType_ScaleBasedTopRight = 0, 
    IconReleaseInfinityViewAnimateType_ScaleBasedTopRight = 0, //default
    /**
     *  scale based the point of upper left corner.
     *  以左上角为基点进行伸缩
     */
    //: IconReleaseInfinityViewAnimateType_ScaleBasedTopLeft,
    IconReleaseInfinityViewAnimateType_ScaleBasedTopLeft,
    /**
     *  scale based the middle point
     *  以中点为基点进行伸缩
     */
    //: IconReleaseInfinityViewAnimateType_ScaleBasedMiddle,
    IconReleaseInfinityViewAnimateType_ScaleBasedMiddle,
    /**
     *  fade in fade out
     *  淡入淡出效果
     */
    //: IconReleaseInfinityViewAnimateType_FadeInFadeOut,
    IconReleaseInfinityViewAnimateType_FadeInFadeOut,
    /**
     *  animate like RollerShutter
     *  卷帘效果
     */
    //: IconReleaseInfinityViewAnimateType_RollerShutter,
    IconReleaseInfinityViewAnimateType_RollerShutter,
    /**
     *  fall from top
     *  从上往下落下
     */
    //: IconReleaseInfinityViewAnimateType_FallFromTop,
    IconReleaseInfinityViewAnimateType_FallFromTop,
//: };
};


//: @protocol IconReleaseInfinityViewDelegate <NSObject>
@protocol IconReleaseInfinityViewDelegate <NSObject>
...
//: - (void)IconReleaseInfinityView:(IconReleaseInfinityView *)menuView WillAppearMenuCell:(IconReleaseInfinityBasedCell *)menuCell index:(NSInteger)index;
- (void)full:(IconReleaseInfinityView *)menuView cell:(IconReleaseInfinityBasedCell *)menuCell betwixt:(NSInteger)index;

//: - (void)IconReleaseInfinityViewWillAppear;
- (void)ownerListener;

//: - (void)IconReleaseInfinityViewWDidAppear;
- (void)cartPolicy;

//: - (void)IconReleaseInfinityViewWillDisappear;
- (void)protectMonitor;

//: - (void)IconReleaseInfinityViewWDidDisappear;
- (void)findWhile;
 ...
 *  下拉菜单
 */
//: @interface IconReleaseInfinityView : UIView
@interface IconReleaseInfinityView : UIView
 ...
 *
 *
 *  @param menuModelsArray     下拉菜单模型数组,数组中存放着(IconReleaseInfinityModel模型对象)
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage IconReleaseInfinityModel instances
...
/** 1、下拉菜单模型数组(数组里面存放 IconReleaseInfinityBasedModel的子类的对象) 
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage IconReleaseInfinityModel instances or the IconReleaseInfinityModel's subclass instances )
 */
...
///   >>步骤:
///   1、 [IconReleaseInfinityView alloc] init]
///   2、 对需要赋值的属性进行赋值
...
/** 2、cell的类名, 必须是IconReleaseInfinityBasedCell的子类
 *               若用框架自带的cell,直接传@"IconReleaseInfinityCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"IconReleaseInfinityCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of IconReleaseInfinityBasedCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"IconReleaseInfinityCell"
 ...
 *
 */
//: @property (nonatomic, weak) id<IconReleaseInfinityViewDelegate> delegate;
@property (nonatomic, weak) id<IconReleaseInfinityViewDelegate> delegate;

/** 15、菜单的弹出的动画效果类型(若不设置，默认为IconReleaseInfinityViewAnimateType_ScaleBasedTopRight)
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     show drop-down menu animate type. (if not assign, default value is IconReleaseInfinityViewAnimateType_ScaleBasedTopRight)
 */
//: @property (nonatomic, assign) IconReleaseInfinityViewAnimateType menuAnimateType;
@property (nonatomic, assign) IconReleaseInfinityViewAnimateType menuAnimateType;
