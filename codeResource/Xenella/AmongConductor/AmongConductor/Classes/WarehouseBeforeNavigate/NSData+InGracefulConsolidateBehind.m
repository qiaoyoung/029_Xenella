...
// __CLOSE_PRINT__
//
//  NSData+InGracefulConsolidateBehind.m
//  NIM
//
...
- (NSData *)readingBind:(NSString *)key {
     ...
     fixedJoin(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
void fixedJoin(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)granted:(NSString *)key secret_strong:(NSString *)vector {
...
- (NSData *)collect:(NSString *)key disabled_strong:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
