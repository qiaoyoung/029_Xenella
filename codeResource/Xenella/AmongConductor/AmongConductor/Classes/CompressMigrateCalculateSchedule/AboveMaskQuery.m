...
- (void)sceneDelegate
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(nonePlace:dismissWith:underForefront:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate nonePlace:self
                                //: currentIndex:_currentPage
                                dismissWith:_currentPage
                                  //: totolPages:[_pages count]];
                                  underForefront:[_pages count]];
    }
}
...
- (void)empty
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(ratePrimary:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource ratePrimary:self];
    }
...
- (void)quality:(UIInterfaceOrientation)toInterfaceOrientation
                                ...
                                overFail:(NSTimeInterval)duration
{
    //: _scrollView.delegate = nil;
    _scrollView.delegate = nil;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (page >= 0 && page < [_pages count])
    {
        ...
        [self contemporary:_currentPage];
    }

    //: if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(pageViewDidScroll:)])
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(volumed:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate volumed:self];
    }
}
...
- (void)spreadhead: (NSInteger)page
{
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(picSpace)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate picSpace];
    }
...
- (void)contemporary:(NSInteger)currentPage
{
    ...
    NSInteger first = [self correctWith:currentPage - 1 minNumbererval:0 observeMagnitude:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self correctWith:currentPage + 1 minNumbererval:0 observeMagnitude:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(highlight:inward:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource highlight:self inward:index];
...
- (NSInteger)correctWith:(NSInteger)value minNumbererval:(NSInteger)min observeMagnitude:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
