...
// __CLOSE_PRINT__
//
//  UIActionSheet+SecureClassRecord.m
//  eim_iphone
//
...
static char moduleOldEvent;


//: @implementation UIActionSheet (SecureClassRecord)
@implementation UIActionSheet (SecureClassRecord)


//: - (void)showInView: (UIView *)view completionHandler: (ActionSheetBlock)block
- (void)random: (UIView *)view sinceOwner: (ActionSheetBlock)block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&moduleOldEvent,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        UITabBar *tabbar = [self aboveCapability];
        //: if (tabbar)
        if (tabbar)
        {
        ...
        else
        {
            ...
            [self performSelector:@selector(linked:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)gray
{
    ...
    objc_setAssociatedObject(self,&moduleOldEvent,nil,OBJC_ASSOCIATION_COPY);
}

//: - (UITabBar *)tabbarForPresent
- (UITabBar *)aboveCapability
{
    //: UITabBar *bar = nil;
    UITabBar *bar = nil;
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &moduleOldEvent) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&moduleOldEvent,nil,OBJC_ASSOCIATION_COPY);
