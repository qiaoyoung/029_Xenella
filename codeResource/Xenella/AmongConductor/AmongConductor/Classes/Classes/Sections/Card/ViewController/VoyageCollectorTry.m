...
//    [self setUpNav];
    ...
    [TupleApplyCompute getWithUrl:Server_user_detail params:dict isShow:NO success:^(id responseObject) {
...
//    [CoordinateProperGridlinePushLanguage show];
    ...
    [TupleApplyCompute getWithUrl:Server_group_getgroups params:nil isShow:NO success:^(id responseObject) {
...
-(void)setingGroup{
    ...
    [TupleApplyCompute getWithUrl:Server_group_addgroupuser params:dict isShow:YES success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
...
- (SignerHastyDawn *)changeRemarksView
{
    if(!_changeRemarksView){
        _changeRemarksView = [[SignerHastyDawn alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_reprotView){
        _reprotView  = [[WorkActionSilverCohesiveUserView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
