...
@interface AAAA_USERSystemNotificationViewController_BBBB ()<NIMSystemNotificationManagerDelegate,AAAA_NIMSystemNotificationCellDelegate_BBBB,NIMTeamManagerDelegate,UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong)    NSMutableArray  *notifications;
@property (nonatomic,assign)    BOOL shouldMarkAsRead;
@end

@implementation AAAA_USERSystemNotificationViewController_BBBB
...
//    AAAA_USERSystemNotificationCell_BBBB *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
//    NIMSystemNotification *notification = [_notifications objectAtIndex:[indexPath row]];
//    [cell update:notification];
//    cell.actionDelegate = self;
//    return cell;
    AAAA_USERSystemNotificationCell_BBBB *cell = [tableView dequeueReusableCellWithIdentifier:@"AAAA_USERSystemNotificationCell_BBBB"];
    if (!cell) {
        cell = [[AAAA_USERSystemNotificationCell_BBBB alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"AAAA_USERSystemNotificationCell_BBBB"];
    }
    cell.actionDelegate = self;
        ...
        case NIMSystemNotificationTypeTeamApply:{
            [[NIMSDK sharedSDK].teamManager passApplyToTeam:notification.targetID userId:notification.sourceID completion:^(NSError *error, NIMTeamApplyStatus applyStatus) {
                if (!error) {
                    ...
                    notification.handleStatus = AAAA_NotificationHandle_BBBBTypeOk;
                    ...
                    if(error.code == NIMRemoteErrorCodeTimeoutError) {
                        ...
                        notification.handleStatus = AAAA_NotificationHandle_BBBBTypeOutOfDate;
                    }
                    [wself.tableView reloadData];
                }
            }];
            ...
            [[[NIMSDK sharedSDK] userManager] requestFriend:request
                                                 completion:^(NSError *error) {
                                                     if (!error) {
                                                         ...
                                                         [AAAA_HttpManager_BBBB postWithUrl:Server_user_addFriend params:@{@"fuid":notification.sourceID?:@""} isShow:NO success:^(id responseObject) {
...
//                                                         // 构造出具体消息
////                                                         NIMMessage *message = [AAAA_USERSessionMsgConverter_BBBB msgWithTip:[NSString stringWithFormat:@"%@%@，%@",LangKey(@"you_have_added"),user.userInfo.nickName,LangKey(@"now_time_chat")]];
            ...
            [[NIMSDK sharedSDK].teamManager rejectApplyToTeam:notification.targetID userId:notification.sourceID rejectReason:@"" completion:^(NSError *error) {
                if (!error) {
                    [wself.navigationController.view makeToast:LangKey(@"message_helper_already_no")
                                                      duration:2
                                                      position:CSToastPositionCenter];
                    notification.handleStatus = AAAA_NotificationHandle_BBBBTypeNo;
                    [wself.tableView reloadData];
                }else {
                    if(error.code == NIMRemoteErrorCodeTimeoutError) {
                        [wself.navigationController.view makeToast:LangKey(@"friend_verify_avtivity_net_error")
