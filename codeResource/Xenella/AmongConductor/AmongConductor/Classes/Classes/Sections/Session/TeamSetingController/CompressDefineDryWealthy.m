...
- (void)updateTeamName{
    ...
    [self.groupnameView animationShow];
        ...
        self.groupnameView.speiceBackBlock = ^(NSString *groupName){
            ...
            [TupleApplyCompute getWithUrl:Server_team_modifyname params:dict isShow:NO success:^(id responseObject) {
                [self reloadData];
...
- (void)dismissTeam {
    ...
    [self.groupAlartView animationShow];
        ...
        self.groupAlartView.speiceBackBlock = ^(NSString *Name){
            @strongify(self)
            [self didDismissTeam];
            [TupleApplyCompute getWithUrl:Server_team_destory params:@{@"id":self.teamListManager.team.teamId} isShow:NO success:^(id responseObject) {
...
- (void)didOntransferWithLeave:(BOOL)isLeave {
    ...
    __block ContactSelectFinishBlock finishBlock =  ^(NSArray * memeber, NSString *name, UIImage *avater){
        ...
        [TupleApplyCompute getWithUrl:Server_team_modifyowner params:dict isShow:NO success:^(id responseObject) {
            [wself reloadData];
        } failed:^(id responseObject, NSError *error) {
...
- (UponConnectStart *)aleartView{
    if(!_aleartView){
        _aleartView = [[UponConnectStart alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
{
    if(!_groupnameView){
        _groupnameView = [[ImplementDarkArrayArmature alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (MixerExpandStrikeDatasetter *)groupNickNameView{
    if(!_groupNickNameView){
        _groupNickNameView = [[MixerExpandStrikeDatasetter alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
...
- (WaitWithoutOperand *)groupAlartView{
    if(!_groupAlartView){
        _groupAlartView = [[WaitWithoutOperand alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
