//
//  AAAA_USERContactAddFriendViewController_BBBB.m
//  NIM
//
...
@interface AAAA_USERContactAddFriendViewController_BBBB ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,AAAA_Recommendfriend_BBBBCellDelegate>
...
@property (nonatomic ,strong) AAAA_ZMONMyQRcodeView_BBBB *MyQRcodeView;
...
@implementation AAAA_USERContactAddFriendViewController_BBBB
...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[AAAA_Recommendfriend_BBBBTableViewCell class] forCellReuseIdentifier:@"AAAA_Recommendfriend_BBBBTableViewCell"];
...
//    dict[@"user_id"] = self.userId;
    [AAAA_HttpManager_BBBB getWithUrl:Server_user_recommend params:nil isShow:NO success:^(id responseObject) {
...
{
    AAAA_Recommendfriend_BBBBTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"AAAA_Recommendfriend_BBBBTableViewCell" forIndexPath:indexPath];
...
-(void)checkIsAdmin:(NSString *)userId{
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_user_checkAddFriendEx params:dict isShow:NO success:^(id responseObject) {
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSString *code = [resultDict newStringValueForKey:@"code"];
        if (code.integerValue == 0) {
            //跳转详情
            ...
            if (isMyFriend) {
                ...
                AAAA_ZMONFriendCardViewController_BBBB *vc = [[AAAA_ZMONFriendCardViewController_BBBB alloc] initWithUserId:userId];
                [wself.navigationController pushViewController:vc animated:YES];
            }
...
- (AAAA_ZMONMyQRcodeView_BBBB *)MyQRcodeView
{
    if(!_MyQRcodeView){
        _MyQRcodeView = [[AAAA_ZMONMyQRcodeView_BBBB alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
