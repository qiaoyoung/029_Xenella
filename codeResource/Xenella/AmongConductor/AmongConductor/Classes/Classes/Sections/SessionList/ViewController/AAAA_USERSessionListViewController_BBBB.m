//
//  AAAA_USERSessionListViewController_BBBB.m
//  NIMDemo
//
...
@interface AAAA_USERSessionListViewController_BBBB ()<NIMLoginManagerDelegate,AAAA_USERListHeader_BBBBDelegate,NIMEventSubscribeManagerDelegate,UIViewControllerPreviewingDelegate,NIMChatExtendManagerDelegate, NIMConversationManagerDelegate,AAAA_USERSystemSignNotificationSheet_BBBBDelegate,NIMSystemNotificationManagerDelegate>
...
@property (nonatomic, strong) AAAA_FFDropDownMenu_BBBBView *dropdownMenu;/** 下拉菜单 */

@property (nonatomic, strong) NSString *creatTeam;

@property (nonatomic, strong) AAAA_USERNotificationView_BBBB *noticeView;
...
@implementation AAAA_USERSessionListViewController_BBBB
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil{
    ...
    if (self) {
        ...
        self.autoRemoveRemoteSession = [[AAAA_USERBundleSetting_BBBB sharedConfig] autoRemoveRemoteSession];
    }
    return self;
}
...
//    [self setupDropDownMenu];
    ...
    self.header = [[AAAA_USERListHeader_BBBB alloc] initWithFrame:CGRectMake(0, 0, self.view.width, 0)];
    ...
    if(![[AAAA_MyUserDefaults_BBBB standardUserDefaults].yspop isEqualToString:@"1"]){
    ...
    [AAAA_HttpManager_BBBB refreshGlobalConfig:^(NSDictionary * _Nonnull configDict) {
        if (configDict.allKeys.count > 0) {
            ...
            if (globalsign.integerValue > 0) {
                AAAA_USERSystemSignNotificationSheet_BBBB *sheet = [[AAAA_USERSystemSignNotificationSheet_BBBB alloc] initWithFrame:self.view.bounds dictionary:@{}];
                sheet.delegate = self;
                [sheet show];
            }
        }
...
- (void)addfriends {
    AAAA_USERContactAddFriendViewController_BBBB *vc = [[AAAA_USERContactAddFriendViewController_BBBB alloc] initWithNibName:nil bundle:nil];
...
-(void)checkCreateTeam{
    ...
    [AAAA_HttpManager_BBBB getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (void)handlerNotice {
    AAAA_USERSystemNotificationViewController_BBBB *vc = [[AAAA_USERSystemNotificationViewController_BBBB alloc] initWithNibName:nil bundle:nil];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)onSelectedRecent:(NIMRecentSession *)recent atIndexPath:(NSIndexPath *)indexPath{
    AAAA_USERSessionViewController_BBBB *vc = [[AAAA_USERSessionViewController_BBBB alloc] initWithSession:recent.session];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)onSelectedAvatar:(NIMRecentSession *)recent
    ...
    if (recent.session.sessionType == NIMSessionTypeP2P) {
        UIViewController *vc;
        vc = [[AAAA_USERPersonalCardViewController_BBBB alloc] initWithUserId:recent.session.sessionId];
        [self.navigationController pushViewController:vc animated:YES];
    }
}
                         ...
                         isTop:(BOOL)isTop
{
    if (isTop)
    {
        ...
        [NIMSDK.sharedSDK.chatExtendManager removeStickTopSession:self.stickTopInfos[recent.session] completion:^(NSError * _Nullable error, NIMStickTopSessionInfo * _Nullable removedInfo) {
            ...
            if (!sself) return;
            if (error) {
                [AAAA_SVProgressHUD_BBBB showErrorWithStatus:error.localizedDescription];
                return;
            }
...
- (void)onLogin:(NIMLoginStep)step {
    ...
    AAAA_AppDelegateManager_BBBB.sharedInstance.loginStep = step;
...
{
//    [self.header refreshWithType:AAAA_USERListHeader_BBBBTypeLoginClients value:[NIMSDK sharedSDK].loginManager.currentLoginClients];
//    [self refreshSubview];
}
...
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (recentSession.session.sessionType == NIMSessionTypeTeam) {
        info = [[AAAA_MyUserKit_BBBB sharedKit] infoByTeam:recentSession.session.sessionId option:nil];
        NIMTeamNotifyState notifyState = [[[NIMSDK sharedSDK] teamManager] notifyStateForNewMsg:info.infoId];
        ...
        info = [[AAAA_MyUserKit_BBBB sharedKit] infoByUser:recentSession.session.sessionId option:option];
        isDisnodistrub = [[NIMSDK sharedSDK].userManager notifyForNewMsg:info.infoId];//判断消息是否勿扰
    }
...
- (NSAttributedString *)contentForRecentSession:(NIMRecentSession *)recent{
    ...
    if (recent.lastMessage.messageType == NIMMessageTypeCustom)
    {
        ...
        NSString *text = [AAAA_USERMessageUtil_BBBB messageContent:msg];
        if (recent.session.sessionType != NIMSessionTypeP2P)
        {
            NSString *nickName = [AAAA_USERSessionUtil_BBBB showNick:msg.from inSession:msg.session];
...
- (NSMutableAttributedString *)transformEmojiDescToEomjiImageWithAttributedString:(NSAttributedString *)attributedString {
    ...
    if (attrM.length > 0) {
        ...
        NSArray<NSTextCheckingResult *> *emoticonResults = [[AAAA_USERSessionListViewController_BBBB regexEmoticon] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
        ...
        [emoticonResults enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSTextCheckingResult * _Nonnull emo, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            AAAA_NIMInputEmoticon_BBBB *emoticon = [[AAAA_FFFInputEmoticonManager_BBBB sharedManager] emoticonByTag:emoString];
...
{
    if ([AAAA_USERSessionUtil_BBBB recentSessionIsMark:recent type:AAAA_USERSessionUtil_BBBBMarkTypeAt]) {
...
{
    if (recent.session.sessionType == NIMSessionTypeP2P) {
//        NSString *state  = [AAAA_USERSessionUtil_BBBB onlineState:recent.session.sessionId detail:NO];
//        if (state.length) {
...
- (void)setupDropDownMenu {
    ...
    self.dropdownMenu = [AAAA_FFDropDownMenu_BBBBView new];
     ...
     /** 菜单的伸缩类型 */
     self.dropdownMenu.menuAnimateType = AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopRight;
     
    self.dropdownMenu.cellClassName = @"AAAA_FFDropDownMenu_BBBBCustomCell";
...
- (NSArray *)getMenuModelsArray {    
    ...
    AAAA_FFDropDownMenu_BBBBModel *menuModel0 = [AAAA_FFDropDownMenu_BBBBModel ff_DropDownMenuModelWithMenuItemTitle:add_friend menuItemIconName:@"ic_add_fiend"  menuBlock:^{
        AAAA_USERContactAddFriendViewController_BBBB *vc = [[AAAA_USERContactAddFriendViewController_BBBB alloc] initWithNibName:nil bundle:nil];
        [self.navigationController pushViewController:vc animated:YES];
    }];
    ...
    //菜单模型1
    AAAA_FFDropDownMenu_BBBBModel *menuModel1 = [AAAA_FFDropDownMenu_BBBBModel ff_DropDownMenuModelWithMenuItemTitle:activity menuItemIconName:@"ic_create_chat" menuBlock:^{
    ...
    //菜单模型1
    @weakify(self);
    AAAA_FFDropDownMenu_BBBBModel *menuModel3 = [AAAA_FFDropDownMenu_BBBBModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
            ...
            if (granted == YES) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
                        AAAA_CCCContactScanViewController_BBBB *vc = [[AAAA_CCCContactScanViewController_BBBB alloc] init];
                        [self.navigationController pushViewController:vc animated:YES];
...
-(void)newGroupSyncRequest:(NSString *)groupName teamID:(NSString *)teamId{
    ...
    dict[@"name"] = groupName;
    dict[@"id"] = teamId;
    [AAAA_HttpManager_BBBB getWithUrl:Server_team_create params:dict isShow:NO success:^(id responseObject) {
...
- (AAAA_USERNotificationView_BBBB *)noticeView
{
    if(!_noticeView){
        _noticeView = [[AAAA_USERNotificationView_BBBB alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
...
- (UIButton *)QuickChatBtn
{
    if (!_QuickChatBtn) {
        ...
        [_QuickChatBtn layoutButtonWithEdgeInsetsStyle:(AAAA_ImageTitleSpacing_BBBBEdgeInsetsStyleLeft) imageTitleSpace:10];
