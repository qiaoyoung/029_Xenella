...
//: @interface TransformStretchLotus ()
@interface TransformStretchLotus ()
//: @property (nonatomic, strong) TransformStretchLotusConfigure *configure;
@property (nonatomic, strong) TransformStretchLotusConfigure *configure;
...
//: @implementation TransformStretchLotus
@implementation TransformStretchLotus
...
- (void)drawRect:(CGRect)rect {
    ...
    [self bound:borderX corner:borderY regularSurplus:cornerLength sheet:insideExcess turnHead:outsideExcess];
    ...
    [self most:borderX bilgeFloat:borderY history:borderH controlWith:cornerLength nontextualMatter:insideExcess minimum:outsideExcess];
    ...
    [self kind:borderX footFloat:borderY followTrust:borderW customBorderExcess:cornerLength validAdded:insideExcess totalerpretationView:outsideExcess];
    ...
    [self rightOrSurplusage:borderX totaleriorise:borderY excess:borderW notice:borderH certificate:cornerLength remote:insideExcess stickAround:outsideExcess];
}
...
- (void)rightOrSurplusage:(CGFloat)borderX totaleriorise:(CGFloat)borderY excess:(CGFloat)borderW notice:(CGFloat)borderH certificate:(CGFloat)cornerLength remote:(CGFloat) insideExcess stickAround:(CGFloat)outsideExcess {
    ...
    //: if (self.configure.cornerLocation == TransformStretchLotusLoactionInside) {
    if (self.configure.cornerLocation == TransformStretchLotusLoactionInside) {
        ...
        [rightBottomPath addLineToPoint:CGPointMake(borderX + borderW - cornerLength - insideExcess, borderY + borderH - insideExcess)];
    //: } else if (self.configure.cornerLocation == TransformStretchLotusLoactionOutside) {
    } else if (self.configure.cornerLocation == TransformStretchLotusLoactionOutside) {
...
- (void)bound:(CGFloat)borderX corner:(CGFloat)borderY regularSurplus:(CGFloat)cornerLength sheet:(CGFloat) insideExcess turnHead:(CGFloat)outsideExcess {
...
- (instancetype)initWithSafely:(CGRect)frame equal:(TransformStretchLotusConfigure *)configure {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        //: [self initialization];
        [self redInitialization];
        //: [self addSubview:self.contentView];
        [self addSubview:self.contentView];

        //: UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tap_action)];
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(firstExecute)];
...
+ (instancetype)before:(CGRect)frame commandFit:(TransformStretchLotusConfigure *)configure {
    //: return [[TransformStretchLotus alloc] initWithFrame:frame configure:configure];
    return [[TransformStretchLotus alloc] initWithSafely:frame equal:configure];
}
...
//: - (void)dealloc {
- (void)dealloc {
    //: if ([LocalDeepPerformLog sharedQRCodeLog].log) {
    if ([LocalDeepPerformLog validContext].log) {
...
- (void)most:(CGFloat)borderX bilgeFloat:(CGFloat)borderY history:(CGFloat)borderH controlWith:(CGFloat)cornerLength nontextualMatter:(CGFloat) insideExcess minimum:(CGFloat)outsideExcess {
...
- (void)kind:(CGFloat)borderX footFloat:(CGFloat)borderY followTrust:(CGFloat)borderW customBorderExcess:(CGFloat)cornerLength validAdded:(CGFloat) insideExcess totalerpretationView:(CGFloat)outsideExcess {
