...
//: @interface SetCancelResumeBasin () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
@interface SetCancelResumeBasin () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
    //: GoodHintedActionRational *soundEffect;
    GoodHintedActionRational *soundEffect;
}
...
//: @implementation SetCancelResumeBasin
@implementation SetCancelResumeBasin
...
//: - (void)setDelegate:(id<SetCancelResumeBasinDelegate>)delegate {
- (void)setDelegate:(id<SetCancelResumeBasinDelegate>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (NSString *)betweenCompositionRate {
...
- (void)outsideValid:(NSString *)name {
    //: NSString *voicePath = [[[MaskRoundTransformerOrchardManager sharedManager] getVoicePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    NSString *voicePath = [[[MaskRoundTransformerOrchardManager signatureNeed] getCap] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    //: soundEffect = [GoodHintedActionRational soundEffectWithFilepath:voicePath];
    soundEffect = [GoodHintedActionRational corral:voicePath];
    //: [soundEffect play];
    [soundEffect full];
}
...
- (instancetype)init {
    //: if ([super init]) {
    if ([super init]) {
        //: self.captureQueue = dispatch_queue_create("com.LocalDeepPerform.captureQueue", DISPATCH_GLOBAL_OBJECT(dispatch_queue_attr_t, _dispatch_queue_attr_concurrent));
        self.captureQueue = dispatch_queue_create("com.LocalDeepPerform.captureQueue", DISPATCH_GLOBAL_OBJECT(dispatch_queue_attr_t, _dispatch_queue_attr_concurrent));
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(multi:compartment:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate multi:self compartment:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(motion:host:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate motion:self host:brightnessValue];
        }
    //: });
    });
}

//: - (void)setSampleBufferDelegate:(id<SetCancelResumeBasinSampleBufferDelegate>)sampleBufferDelegate {
- (void)setSampleBufferDelegate:(id<SetCancelResumeBasinSampleBufferDelegate>)sampleBufferDelegate {
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self betweenCompositionRate];
    }
    //: return _session;
    return _session;
}
