...
@interface RiderReplaceData : NSObject

@end

@implementation RiderReplaceData

+ (NSData *)RiderReplaceDataToData:(NSString *)value {
...
+ (NSString *)componentDependingMessage {
    /* static */ NSString *componentDependingMessage = nil;
    if (!componentDependingMessage) {
		NSString *origin = @"1A0708AA92A549925F6B625E675D585C626B5C655E585A5D5A696D5E6B586562645EE9";
		NSData *data = [RiderReplaceData RiderReplaceDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        componentDependingMessage = [self StringFromRiderReplaceData:value];
    }
    return componentDependingMessage;
}

//: ic_fold
+ (NSString *)themeTargetPage {
    /* static */ NSString *themeTargetPage = nil;
    if (!themeTargetPage) {
		NSString *origin = @"07380D6191F0E964D73113F873312B272E37342C72";
		NSData *data = [RiderReplaceData RiderReplaceDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        themeTargetPage = [self StringFromRiderReplaceData:value];
    }
    return themeTargetPage;
}

//: #333333
+ (NSString *)layoutRetailHelper {
    /* static */ NSString *layoutRetailHelper = nil;
    if (!layoutRetailHelper) {
		NSString *origin = @"072307097CD3EA0010101010101032";
		NSData *data = [RiderReplaceData RiderReplaceDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        layoutRetailHelper = [self StringFromRiderReplaceData:value];
    }
    return layoutRetailHelper;
}

//: arrowDown
+ (NSString *)kTacticPreference {
    /* static */ NSString *kTacticPreference = nil;
    if (!kTacticPreference) {
		NSString *origin = @"0932032F40403D45123D453C3B";
		NSData *data = [RiderReplaceData RiderReplaceDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        kTacticPreference = [self StringFromRiderReplaceData:value];
    }
    return kTacticPreference;
}

+ (Byte *)RiderReplaceDataToCache:(Byte *)data {
    int playWeekend = data[0];
    Byte magnitudeerval = data[1];
    int nameBlank = data[2];
    for (int i = nameBlank; i < nameBlank + playWeekend; i++) {
        int value = data[i] + magnitudeerval;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[nameBlank + playWeekend] = 0;
    return data + nameBlank;
}

//: ic_praise%d
+ (NSString *)kTrailerPreference {
    /* static */ NSString *kTrailerPreference = nil;
    if (!kTrailerPreference) {
		NSString *origin = @"0B5705F21D120C08191B0A121C0ECE0D96";
		NSData *data = [RiderReplaceData RiderReplaceDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        kTrailerPreference = [self StringFromRiderReplaceData:value];
    }
    return kTrailerPreference;
}

//: FFFCF7
+ (NSString *)viewConvinceContent {
    /* static */ NSString *viewConvinceContent = nil;
    if (!viewConvinceContent) {
		NSString *origin = @"06520C7292B6315B7383466CF4F4F4F1F4E532";
		NSData *data = [RiderReplaceData RiderReplaceDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        viewConvinceContent = [self StringFromRiderReplaceData:value];
    }
    return viewConvinceContent;
}

//: arrowUp
+ (NSString *)featureFlightId {
    /* static */ NSString *featureFlightId = nil;
    if (!featureFlightId) {
		NSString *origin = @"07150DEDF2E8480D4EBE9A61B24C5D5D5A62405BF5";
		NSData *data = [RiderReplaceData RiderReplaceDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        featureFlightId = [self StringFromRiderReplaceData:value];
    }
    return featureFlightId;
}

+ (NSString *)StringFromRiderReplaceData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self RiderReplaceDataToCache:data]];
}
...
//: typedef NS_OPTIONS(NSUInteger, BeyondPlayfulGardenReset) {
typedef NS_OPTIONS(NSUInteger, BeyondPlayfulGardenReset) {
    //: BeyondPlayfulGardenReset_Up,
    BeyondPlayfulGardenReset_Up,
    //: BeyondPlayfulGardenReset_Down
    BeyondPlayfulGardenReset_Down
//: };
};

//: @interface MagicBindProvisionLink()
@interface MagicBindProvisionLink()
...
//: @property (nonatomic, assign)BeyondPlayfulGardenReset directionPriority;
@property (nonatomic, assign)BeyondPlayfulGardenReset directionPriority;
...
//: @implementation MagicBindProvisionLink
@implementation MagicBindProvisionLink

//: - (void)onButtonTouched:(UIButton *)button {
- (void)albums:(UIButton *)button {
    //: if (_myNewContentArray.count > button.tag - 100) {
    if (_myNewContentArray.count > button.tag - 100) {
        //: CoreWorldSearch *model = (CoreWorldSearch *)_myNewContentArray[button.tag - 100];
        CoreWorldSearch *model = (CoreWorldSearch *)_myNewContentArray[button.tag - 100];


        //: if([model.name isEqual:[IsletSavePreview getTextWithKey:@"friend_circle_adapter_like"]]){
        if([model.name isEqual:[IsletSavePreview being:[RiderReplaceData componentDependingMessage]]]){
...
- (UIView *)withElement:(NSArray *)array {
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        [button addTarget:self action:@selector(albums:) forControlEvents:UIControlEventTouchUpInside];

        //: CoreWorldSearch *model = array[i];
        CoreWorldSearch *model = array[i];
        ...
        label.textColor = [UIColor extra:[RiderReplaceData layoutRetailHelper]];
    ...
    _buttonsBgView.backgroundColor = [UIColor extra:[RiderReplaceData viewConvinceContent]];
...
+ (instancetype)refuse {
    //: static MagicBindProvisionLink *menu = nil;
    static MagicBindProvisionLink *menu = nil;
    ...
    _dispatch_once(&onceToken, ^{
        //: if (menu == nil) {
        if (menu == nil) {
            //: menu = [[MagicBindProvisionLink alloc] init];
            menu = [[MagicBindProvisionLink alloc] init];
        }
    //: });
    });
...
- (void)generaling:(UIButton *)button {
...
- (void)trademark:(NSArray *)array
                 //: cursorStartRect:(CGRect)cursorStartRect selectionTextRectInWindow:(CGRect)rect selectBlock:(void(^)(CalibrateReturnDevice *item))block praiseBlock:(void(^)(NSInteger tag))praiseblock {
                 sustainBlock:(CGRect)cursorStartRect chemicalAgent:(CGRect)rect enter:(void(^)(CalibrateReturnDevice *item))block surplusBlock:(void(^)(NSInteger tag))praiseblock {
    ...
    UIView *buttonsBgView = [self withElement:array];
    ...
    //默认的方向是优先方向
    //: BeyondPlayfulGardenReset direction = _directionPriority;
    BeyondPlayfulGardenReset direction = _directionPriority;
//
...
//        //优先级Up的话，先判断在上面的情况。
        ...
        if (rect.origin.y - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            selfRect.origin.y = rect.origin.y - _bubbleContentMargin - _bubbleHeight - praiseheight + 15;
            //: direction = BeyondPlayfulGardenReset_Up;
            direction = BeyondPlayfulGardenReset_Up;
        ...
        //: } else if (screenHeight - rect.origin.y - rect.size.height - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            selfRect.origin.y = rect.origin.y + rect.size.height + _bubbleContentMargin -15;
            //: direction = BeyondPlayfulGardenReset_Down;
            direction = BeyondPlayfulGardenReset_Down;
...
//    }
    ...
    CGRect bgViewRect = buttonsBgView.frame;
    //: bgViewRect.origin.y = direction == BeyondPlayfulGardenReset_Down ? _arrowHeight : praiseheight;
    bgViewRect.origin.y = direction == BeyondPlayfulGardenReset_Down ? _arrowHeight : praiseheight;
...
//    }
    ...
    //: if (direction == BeyondPlayfulGardenReset_Up) {
    if (direction == BeyondPlayfulGardenReset_Up) {
        ...
        _arrowView.image = [UIImage imageNamed:[RiderReplaceData kTacticPreference]];
    ...
    //: } else {
        ...
        _arrowView.image = [UIImage imageNamed:[RiderReplaceData featureFlightId]];
...
- (void)castWoman{
    ...
    for (int i = 0; i < _buttonCount; i ++) {
        ...
        if(_isfold && i==7){
            ...
            [_foldbtn addTarget:self action:@selector(completeClick:) forControlEvents:UIControlEventTouchUpInside];
            //: [_foldbtn setImage:[UIImage imageNamed:@"ic_fold"] forState:UIControlStateNormal];
            [_foldbtn setImage:[UIImage imageNamed:[RiderReplaceData themeTargetPage]] forState:UIControlStateNormal];
        ...
        [button addTarget:self action:@selector(generaling:) forControlEvents:UIControlEventTouchUpInside];
        //: [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"ic_praise%d",i+1]] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:[RiderReplaceData kTrailerPreference],i+1]] forState:UIControlStateNormal];
...
- (BOOL)radio:(NSArray *)newArray {
    ...
    for (int i = 0; i < newArray.count; i ++) {
        //: BOOL finded = false;
        BOOL finded = false;
        //: NSInteger newId = ((CoreWorldSearch *)_oldContentArray[i]).buttonId;
        NSInteger newId = ((CoreWorldSearch *)_oldContentArray[i]).buttonId;
        //: for (int j = 0; j < _oldContentArray.count; j ++) {
        for (int j = 0; j < _oldContentArray.count; j ++) {
            //: NSInteger oldId = ((CoreWorldSearch *)_oldContentArray[j]).buttonId;
            NSInteger oldId = ((CoreWorldSearch *)_oldContentArray[j]).buttonId;
            //: if (newId == oldId) {
            if (newId == oldId) {
                //: finded = true;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _bubbleBottomMargin = 20;
        //: _directionPriority = BeyondPlayfulGardenReset_Up;
        _directionPriority = BeyondPlayfulGardenReset_Up;

        //: UITapGestureRecognizer *singleTapbox1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(clickView)];
        UITapGestureRecognizer *singleTapbox1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(editReject)];
