...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [CoordinateProperGridlinePushLanguage showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (ObviousResilientAt *)selectIndicatorView{
    ...
    _selectIndicatorView = [[ObviousResilientAt alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (TerminalOperatorSurface *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[TerminalOperatorSurface alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
