...
+ (nonnull LEEActionSheetConfig *)keepTable;
...
+ (void)albumses:(void (^ _Nullable)(void))completionBlock;
...
+ (nonnull LEEAlertConfig *)clingUser;
...
+ (void)greenish:(NSString *)identifier table:(BOOL)force drawing:(void (^ _Nullable)(void))completionBlock;
...
@property (nonatomic, copy, readonly) LEEConfigToUserInterfaceStyle LeeUserInterfaceStyle
...
+ (LEEPresentation *)yearBecome: (UIViewController *)controller;

//: + (LEEPresentation *)windowLevel: (UIWindowLevel)level;
+ (LEEPresentation *)write: (UIWindowLevel)level;
...
CornerRadii mergeZero(void);

//: CornerRadii CornerRadiiNull(void);
CornerRadii teamsterNull(void);
