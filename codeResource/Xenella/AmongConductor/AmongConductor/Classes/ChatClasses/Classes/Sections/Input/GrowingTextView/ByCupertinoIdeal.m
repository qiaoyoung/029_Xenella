...
//: \n|W|
@property (nonatomic, copy) NSString *widgetSnowConfig;
...
//: \n|W|
- (NSString *)widgetSnowConfig {
    if (!_widgetSnowConfig) {
		NSArray<NSString *> *origin = @[@"4", @"86", @"5", @"43", @"110", @"180", @"38", @"1", @"38", @"124"];
		NSData *data = [DueData DueDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _widgetSnowConfig = [self StringFromDueData:value];
    }
    return _widgetSnowConfig;
}

- (Byte *)DueDataToCache:(Byte *)data {
    int steadilyPartRoutine = data[0];
    Byte socialDancing = data[1];
    int diplomaticInstruct = data[2];
    for (int i = diplomaticInstruct; i < diplomaticInstruct + steadilyPartRoutine; i++) {
        int value = data[i] + socialDancing;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[diplomaticInstruct + steadilyPartRoutine] = 0;
    return data + diplomaticInstruct;
}

+ (NSData *)DueDataToData:(NSArray<NSString *> *)value {
...
//: @interface ByCupertinoIdeal()<UITextViewDelegate>
@interface ByCupertinoIdeal()<UITextViewDelegate>

//: @property (nonatomic,strong) AnimateHeaderFrameworkDecorator *textView;
@property (nonatomic,strong) AnimateHeaderFrameworkDecorator *textView;
...
//: @implementation ByCupertinoIdeal
@implementation ByCupertinoIdeal
...
- (void)hide{
    ...
    if (newScrollViewFrame.size.height <= self.maxHeight && _previousTextHeight == 0) {
        ...
        if(oldScrollViewFrame.size.height != newScrollViewFrame.size.height) {
            ...
            if ([self.textViewDelegate respondsToSelector:@selector(lieInWait:)]) {
                //: [self.textViewDelegate willChangeHeight:newScrollViewFrame.size.height];
                [self.textViewDelegate lieInWait:newScrollViewFrame.size.height];
            }
    ...
    if (oldScrollViewFrame.size.height != newScrollViewFrame.size.height && [self.textViewDelegate respondsToSelector:@selector(automatics:)]) {
        //: [self.textViewDelegate didChangeHeight:newScrollViewFrame.size.height];
        [self.textViewDelegate automatics:newScrollViewFrame.size.height];
    }
...
- (BOOL)textViewShouldEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(stackses:)]) {
        //: return [self.textViewDelegate textViewShouldEndEditing:self];
        return [self.textViewDelegate stackses:self];
    }
    //: return YES;
    return YES;
}
...
- (void)setMinNumberOfLines:(NSInteger)minNumberOfLines
{
    ...
    self.minHeight = [self heightRunning:minNumberOfLines];
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(enterAcrossQuery:)]) {
       //: return [self.textViewDelegate textViewShouldBeginEditing:self];
       return [self.textViewDelegate enterAcrossQuery:self];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(distance:agree:)]) {
        //: return [self.textViewDelegate shouldInteractWithURL:URL inRange:characterRange];
        return [self.textViewDelegate distance:URL agree:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(mied:)]) {
        //: [self.textViewDelegate textViewDidBeginEditing:self];
        [self.textViewDelegate mied:self];
    }
}
...
- (void)setMaxNumberOfLines:(NSInteger)maxNumberOfLines
{
    ...
    self.maxHeight = [self heightRunning:maxNumberOfLines];
    //: _maxNumberOfLines = maxNumberOfLines;
    _maxNumberOfLines = maxNumberOfLines;
}

//: - (CGFloat)simulateHeight:(NSInteger)line
- (CGFloat)heightRunning:(NSInteger)line
{
    ...
    for (NSInteger index=0; index<line; index++) {
        ...
        [newText appendString:[DueData sharedInstance].widgetSnowConfig];
    }
    ...
    CGFloat height = self.airship.height - (textViewMargin + self.textView.contentInset.top + self.textView.contentInset.bottom);
...
- (CGSize)intrinsicContentSize
{
    ...
    return [self agree:self.airship].size;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    ...
    //: if (self) {
    if (self) {
        //: AnimateHeaderFrameworkDecorator *textView = [[AnimateHeaderFrameworkDecorator alloc] initWithFrame:CGRectZero];
        AnimateHeaderFrameworkDecorator *textView = [[AnimateHeaderFrameworkDecorator alloc] initWithFrame:CGRectZero];
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithTextAttachment:(NSTextAttachment *)textAttachment inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(method:writtenMatter:)]) {
        //: return [self.textViewDelegate shouldInteractWithTextAttachment:textAttachment inRange:characterRange];
        return [self.textViewDelegate method:textAttachment writtenMatter:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGRect rect = CGRectMake(0, 0, frame.size.width, frame.size.height);
        //: AnimateHeaderFrameworkDecorator *textView = [[AnimateHeaderFrameworkDecorator alloc] initWithFrame:rect];
        AnimateHeaderFrameworkDecorator *textView = [[AnimateHeaderFrameworkDecorator alloc] initWithFrame:rect];
        //: self.textView = textView;
        self.textView = textView;
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(dispersedPhaseTitle:alteration_strong:)]) {
        //: return [self.textViewDelegate shouldChangeTextInRange:range replacementText:text];
        return [self.textViewDelegate dispersedPhaseTitle:range alteration_strong:text];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(outerSpace:)]) {
        //: [self.textViewDelegate textViewDidChangeSelection:self];
        [self.textViewDelegate outerSpace:self];
    }
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(findses:)]) {
        //: [self.textViewDelegate textViewDidChange:self];
        [self.textViewDelegate findses:self];
    }
    //: [self fitToScrollView];
    [self hide];
}
...
- (CGRect)agree:(CGSize)contentSize
{
    //: CGSize selfSize;
    CGSize selfSize;
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(progresses:)]) {
        //: [self.textViewDelegate textViewDidEndEditing:self];
        [self.textViewDelegate progresses:self];
    }
}
...
//: @implementation ByCupertinoIdeal(TextView)
@implementation ByCupertinoIdeal(TextView)
