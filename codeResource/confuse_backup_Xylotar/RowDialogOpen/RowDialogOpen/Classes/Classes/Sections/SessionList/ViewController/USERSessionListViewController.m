//
//  USERSessionListViewController.m
//  NIMDemo
//
...
-(void)checkCreateTeam{
    ...
    [HttpManager getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
{
    if (recent.session.sessionType == NIMSessionTypeP2P) {
//        NSString *state  = [USERSessionUtil onlineState:recent.session.sessionId detail:NO];
//        if (state.length) {
...
{
    if(!_noticeView){
        _noticeView = [[USERNotificationView alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
